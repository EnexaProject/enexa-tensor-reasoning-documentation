...
\begin{align}\label{eq:hlnMeansetConvCharacterization}
	\meansetof{\formulaset} 
	= \convhullof{\sencodingofat{\formulaset}{\indexedshortcatvariables,\selvariable} \, : \, \shortcatindices\in\atomstates} \, . 
\end{align}
...
\subsubsection{Interior points by Markov Logic Networks}

\red{Following the Theorem 3.3. of Wainwright, cite here the ProbReasoning chapter version.}



\subsubsection{Points outside the interior}
...
\begin{corollary}[of Theorem~\ref{the:maxEntMaxLikeDuality}]
	%\red{Works only for $\meanparamat{\indexedselvariable}\in\interiorof{\meansetof{\formulaset}}$}
