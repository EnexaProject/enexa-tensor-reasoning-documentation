...
%\drawatomindices{7}{-8}	
	...
	\begin{scope}[shift={(0,-10)}]
		...
		\draw[->] (5.5,5) -- (5.5,7) node[midway, right] {\tiny ${\atomicformulaof{\parindexof{1}} \land \atomicformulaof{\parindexof{2}}}$};	
		\draw (1,3) rectangle (10, 5);
		\node[anchor=center] (text) at (5.5,4) {$\rencodingof{\land}$};

			
		% SelectorCores
		\draw[->] (2,1) -- (2,3) node[midway, left] {\tiny ${\atomicformulaof{\parindexof{2}}}$};
		...
		\draw (9,-1) -- (9,1);
		\draw[->] (9,1) -- (9,3) node[midway, left] {\tiny ${\atomicformulaof{\parindexof{1}}}$};
		...
		\node[anchor=center] (text) at (15,-1) {$\parametertensor$};
