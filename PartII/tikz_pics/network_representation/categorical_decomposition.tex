...
\node [circle, draw, thick, fill=gray!50] (T2) at (3,0) {\tiny $\catvariableof{1}$};
\node[anchor=center] (text) at (6,0) {\small $\cdots$};
\node [circle, draw, thick, fill=gray!50] (T3) at (9,0) {\tiny $\catvariableof{\atomorder-1}$};

\node [circle, draw, thick, fill=gray!50] (C) at (4.5,-5) {\tiny $\catvariableof{\categoricalmap}$};
...
\draw[->] (2,-3) -- (2,-1) node[midway,left] {\tiny $\catvariableof{\categoricalmap}$};
...
\draw (\conposseldec) -- (4.5,-7.5) node[midway, right] {\tiny $\catvariableof{\categoricalmap}$};
