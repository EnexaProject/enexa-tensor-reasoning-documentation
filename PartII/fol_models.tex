...
\begin{definition}[FOL World]\label{def:folWorld}
	...
	%\begin{align*}
	%	\folpredicateof{\folpredicateenumerator}(\individuals) = \dataworldat{\indexedselvariable,\indexedshortindvariables} \, .
	%\end{align*}
\end{definition}
...
\subsect{Special Case of Propositional Logics}

% 
Propositional Logics is the case of $\indorder=0$.
In that case, worlds are
	\[ \dataworld: [\catorder] \rightarrow [2] \]
and represented by the tuples $\catindexof{\atomenumerator} = \dataworld(\atomenumerator)$ as before.
...
\begin{definition}[Grounding of a first-order formula given a world]
	Given a specific world $\dataworld$, with an domain $\worlddomain$ enumerated by $[\inddim]$, the grounding of a formula $\folexformula$ with variables $\individualorder_{\folexformula}$  is the tensor
	\begin{align*}
	...
	\begin{align*}
		\groundingofat{\folexformula}{\indexedindvariableof{0},\ldots,\indexedindvariableof{\individualorder_{\folexformula}-1}} =
			\begin{cases}
				1 & \quad \text{if the formula $\folexformula$ is satisfied on the world $\dataworld$, when substituing its variables with the objects $\indexinterpretationat{\indindexof{\indenumerator}}$.} \\
				%$\folexformula\left(\exindividualof{\indindexof{0}},\ldots,\exindividualof{\indindexof{\individualorder_{\folexformula}-1}}\right)=1$ given the world $\dataworld$}  \\
				0 &  \text{else}
			\end{cases} \, . 
	\end{align*}
\end{definition}
...
\subsect{Basis CP Decomposition}\label{sec:basisCPgrounding}
...
\subsect{Example: Queries}

\red{A database is understood as a specific fist order logic world.
Querying is an operation on such a single world and therefore exploits the first level of tensor representation.}
...
%Visualization
We can thus understand the $\mathrm{Join}$ of groundings by a factor graph, where each grounding is a factor connecting the variables decorating node sets $\nodes_l$.
The $\mathrm{Join}$ can be stored by at most linear in the number of joined formulas demand.
...
\sect{Representation of Knowledge Graphs}
...
\subsect{Representation as ternary predicate}
...
%\subsect{Construction of Empirical Distributions from groundings of Sentences}
%
...
%% OLD: Do 
%\subsect{FOL grounding tensors as Knowledge Graphs}
%
