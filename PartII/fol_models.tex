...
\begin{definition}[World enumerating variables]\label{def:worldEnumeratingVariables}
...
%Before continuing with the one-hot encoding of \firstOrderLogic{} worlds, let us show that the previously discussed formalism of \propositionalLogic{} (see \charef{cha:logicalRepresentation}) is a special case of \firstOrderLogic{}, namely when demanding $\indorder=0$.
%Consistent with \defref{def:folWorld} we have a propositional logic world by an index
%\begin{align*}
%    \worldindices: [\catorder] \rightarrow [2] \, ,
%\end{align*}
...
\begin{align*}
    ...
    = \bigotimes_{\catenumeratorin} \onehotmapofat{\dataworldat{\selvariable=\atomenumerator}}{\catvariableof{\catenumerator}} \, .
\end{align*}
...
defined by
\begin{align*}
    \onehotmapofat{\worldindices}{\catvariableof{[\catorder]\times[\inddim]^{\indorder}}}
...
Probability distributions over worlds coinciding on their domain are then non-negative and normed tensors $\probat{\worldvariables}$
%\begin{align*}
...
%\end{align*}
where each coordinate of a world $\worldindices$ is captured by a boolean random variable $\catvariableof{\atomenumerator,\shortindindices}$, indicating whether the $\atomenumerator$-th predicate holds on the object tuple indexed by $\shortindindices$.
