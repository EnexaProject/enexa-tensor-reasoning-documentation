\section{Statistical Models of First Order Logic (FOL)}\label{cha:folModels}
...
% Templates
Following \cite{domingos_markov_2006} we define Markov Logic Networks as templates for distributions, which instantiate random worlds when choosing a set of objects $\worlddomain$.
...
\begin{definition}[Markov Logic Networks in FOL]
	...
	\begin{align*}
		\sstat_{\statenumerator}(\dataworld)  = \sbcontraction{\groundingof{\folexformula_\statenumerator}} % Formulas can have different 
	\end{align*}
...
The statistics
	\begin{align*}
		\sbcontraction{\groundingof{\folexformula_\statenumerator}} % Formulas can have different 
	\end{align*}
...
%\begin{definition}
%	We say that $\impformula,\atomicqueries$ do not influence each other, when for any pair $\individuals$, $\atomenumerator,\dataindex$ and $\sigma : [\atomorder] \times [\datanum] \rightarrow [2]$ the formula 
%		\[ \left( \land_{\dataindexin} \impformula(\individualsof{\dataindex}) \right)
%		\land 
%		\left( \land_{\atomenumeratorin, \dataindexin, \dataindex\neq\secdataindex, \atomenumerator\neq\secatomenumerator} (\lnot)^{\sigma(\atomenumerator,\dataindex)}\extformulaof{\atomenumerator}(\individualsof{\dataindex})  \right) 
%		\Rightarrow
%		 (\lnot)^{\sigma(\secatomenumerator,\secdataindex)} \extformulaof{\secatomenumerator}(\individualsof{\secdataindex}) 
%		 \]
%	is not a tautology.
%\end{definition}
%This is satisfied, when different predicates are in $\impformula$ and $\atomicqueries$, both are quantor and constant free, and all individual pairs with $\impformula(\individuals)=1$ are disjoint.
%
%
%%
...
%\begin{theorem}
%	When $\impformula,\atomicqueries$ do not influence each other we have
%		\[ \frac{1}{\datanum}\lnof{\probtensor^{\folformulaset,\weight,\worlddomain,\fixedimpformula}[\dataworld]} = 
%		\lossof{\formulaset,\weight} \]
%%		\prod_{\dataindexin} \left( \probtensor^{\formulaset,\weight}[\extformulaof{0}(\individualsof{\dataindex})] \right)   \]
%	where 
%		\[ \datamap(\dataindex) = \{(\groundingof{\extformulaof{0}}(\individualsof{\dataindex}),\ldots, \groundingof{\extformulaof{\atomorder-1}}(\individualsof{\dataindex}) ) : \dataindexin \}\]
%	and $\individualsof{\dataindex}$ is the enumeration of the pairs satisfying $\impformula(\individuals)=1$.
%\end{theorem}
%\begin{proof}
...
%	defined as
%		\[ T^\dataworld(\atomenumerator,\dataindex) = \groundingof{\extformulaof{\atomenumerator}}(\individualsof{\dataindex}) \, .  \]
%	By the assumption, that $\impformula,\atomicqueries$ do not influence each other, each boolean tensor $T: [\atomorder] \times [\datanum] \rightarrow [2]$ has a world $\dataworld$ with $T^{\dataworld}=T$.
%	
%	The partition function is then factorized into
%	\begin{align*}
%		\secpartitionfunctionof{\folformulaset,\weight,\worlddomain,\fixedimpformula} 
%		=&  \sum_{\groundingof{\extformulaof{\atomenumerator}}(\individualsof{\dataindex})\in[2],\atomenumeratorin,\dataindexin}
%			\prod_{\dataindexin} \expof{\sum_{\folexformulain} \folexformula(\individualsof{\dataindex}) \cdot \weightof{\folexformula}}
%		 \\
%		=& \prod_{\dataindexin} \left( \sum_{\groundingof{\extformulaof{\atomenumerator}}(\individualsof{\dataindex}) \in [2], \atomenumeratorin} 
%			\expof{\sum_{\folexformulain} \folexformula(\individualsof{\dataindex}) \cdot \weightof{\folexformula}}
%		\right) \\
...
%	\begin{align*}
%		\frac{1}{\datanum}\lnof{\probtensor^{\folformulaset,\weight,\worlddomain,\fixedimpformula}[\dataworld]}
%		=& \frac{1}{\datanum} \left[ \sum_{\dataindexin} \sum_{\folexformulain} \folexformula(\individualsof{\dataindex}) \cdot \weightof{\folexformula} + \lnof{\secpartitionfunctionof{\folformulaset,\weight,\worlddomain,\fixedimpformula}} \right] \\
%		=& \frac{1}{\datanum}\sum_{\dataindexin} \sum_{\folexformulain} \contractionof{\{\exformula,\onehotmapof{\groundingof{\extformulaof{0}}(\individualsof{\dataindex})},\ldots,\onehotmapof{\groundingof{\extformulaof{\atomorder-1}}(\individualsof{\dataindex})}\}}{\varnothing}  \cdot \weightof{\folexformula}
%		+ \frac{1}{\datanum} \left( \datanum \cdot \lnof{\partitionfunctionof{\formulaset,\weight}}  \right) \\
%		=& \frac{1}{\datanum} \left[ \sum_{\dataindexin} \sum_{\exformulain} \exformula\big(\groundingof{\extformulaof{0}}(\individualsof{\dataindex}),\ldots, \groundingof{\extformulaof{\atomorder-1}}(\individualsof{\dataindex})\big) \cdot \weightof{\folexformula} \right]
...
In general, we cannot assume that the $\impformula,\atomicqueries$ do not influence each other.
...
%We can further utilize any decomposition of $\impformula$ into a directed and binary CP Format to enumerate the datapoints by the slice index $\dataindex$. % Approaches like SPARQL directly give us these by solution mappings.
...
The atoms are then the atomizations of this variable (see Section~\ref{sec:categoricalTN}), treated as a categorical variable, with respect to the constant in the extraction query.
...
%
%When taking groundings of sentences instead, we have multiple data axis then $\dataindex$, one for each variable in the sentence.
%
...
%	Enumerating the individual tuples $a_1,...,a_n$ satisfying the query introduced the dataindex $\dataindex$, which is averaged over.
%\end{remark} 
...
%
%We can represent any grounding tensor as a knowledge graph where the individuals are enumerated by $\dataindex$ and the classes are the arguments enumerated by $\indenumerator$.
%Given the naive CP decomposition (Figure~\ref{fig:groundingCP}) of a grounding tensor we define for any $\dataindex$ and $\indenumerator$
%\begin{align}
%	\gtensorof{\kg}_{\dataindex,0,\indenumerator} := \gtensorof{\indenumerator}_{\dataindex,1} \, .
%\end{align}
%
%\begin{example}[Knowledge Graphs out of samples]
%	We might have a single data index $\dataindex$ marking different sample worlds.
