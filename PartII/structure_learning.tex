...
\begin{align*}
	 \exfunction(\canparam) %=  \mlntensor + \weight_{\parindices} \ftensorof{\exformula_{\parindices}}
...
\begin{theorem}
	Problem~\eqref{prob:greedyGain}
	%~\ref{prob:restrictedNaiveMLE} in the case 
	%\begin{align*}
	%	\energyhypothesis =  \lnof{\currentdistribution} + \bigcup_{\formula\in\formulaset} \spanof{\formula} 
	%\end{align*}
	is solved at any
	\begin{align*}
...
\begin{proof}
	...
	For any formula $\formula$, the inner minimum of Problem~\eqref{prob:greedyGain} is by Lemma~\ref{ref:lemMMinMLN} taken at %Problem~\ref{prob:restrictedNaiveCE}  in the case $\energyhypothesis = \spanof{\formula} $ 
