...
%	\[ \bencodingof{\circ}_{\selindex,:,:} = \bencodingof{\circ_{\selindex}}_{:,:} \, . \]
...
%Using that the encoding $\bencodingof{\atomicformulaof{\selindex}}$ of atomic formulas admits and elementary decomposition (see \theref{the:AtomicFTensor}) we notice that Equation~\ref{eq:selectorDecomposition} describes a so-called monomial decomposition, which will be introduced in \defref{def:polynomialSparsity}.
%We can apply \theref{the:sliceToCP} to find a decomposition of $\selectorcore$ in a CP format consisting of cores
%\begin{align}
%	\selectorcoreof{\selindex} = \dirdeltaof{\catvariableof{\selindex},\selvariable} \otimes \onehotmapof{\selindex}
%	+ \sum_{\tilde{\selindex}\in[\seldim] \, , \, \tilde{\selindex}\neq\selindex} \onesof{\catvariableof{\selindex},\selvariable} \otimes \onehotmapof{\tilde{\selindex}} \, .
%\end{align}	
...
%	{\{\catvariableof{0},\ldots,\catvariableof{\seldim-1},\selvariable\}}
%\end{align}
