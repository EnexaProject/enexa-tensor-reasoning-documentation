...
$(\variableset,\headindexof{\variableset},\canparam)$ as follows
\begin{itemize}
    ...
    \begin{align*}
        ...
        \begin{cases}
            0 & \text{if } \hypercoreofat{\selindex}{\headvariableof{\selindex}=1}=0 \\
            1 & \text{if } \hypercoreofat{\selindex}{\headvariableof{\selindex}=0}=0
        \end{cases}
    \end{align*}
    ...
    \end{align*}
\end{itemize}
Conversely, for a tuple $(\variableset,\headindexof{\variableset},\canparam)$ we define leg vectors
\begin{align*}
...
\begin{align*}
    \tnetofat{(\variableset,\headindexof{\variableset},\canparam)}{\headvariables}
    =\bigotimes_{\selindexin}\hypercoreofat{\selindex}{\headvariableof{\selindex}} \, .
\end{align*}

Note that the tuple $(\variableset,\headindexof{\variableset},\canparam)$ to a elementary activation tensor determines the corresponding \HybridLogicNetwork{}, since it determines the activation tensor up to an irrelevant scalar multiplication.
...
\begin{definition}
    ...
    \begin{align*}
        ...
        \quad \text{where} \quad
        \forwardmapwrtof{\mlnstat}{(\variableset,\headindexof{\variableset},\canparam)}
        = \contractionof{
            \normalizationof{\tnetofat{(\variableset,\headindexof{\variableset},\canparam)}{\headvariables},\bencodingofat{\mlnstat}{\headvariables,\shortcatvariables}}{\shortcatvariables}
    ...
    %\end{align*}
    such that for any $(\variableset,\headindexof{\variableset},\canparam)\in\mathcal{N}_{\seldim}$ we have $\backwardmapwrtof{\mlnstat}{\forwardmapwrtof{\mlnstat}{\tnetof{\elgraph}}}$.

\end{definition}

From the expressivity study of \charef{cha:networkRepresentation} we have the following.
...
\subsect{Alternating Algorithms for Backward Maps}\label{sec:alternatingParEstMLN}
...
\begin{lemma}
    The moment matching conditions of \lemref{ref:lemMMinMLN} are satisfied for some $\canparamwith$, if and only if $\meanparamwith\in\sbinteriorof{\hlnmeanset}$.
\end{lemma}
...
In the initialization phase of \algoref{alg:AMM_MLN}, each parameters is initialized relative to a uniform distribution.
...
\begin{lemma}
    The moment matching conditions of \lemref{ref:lemMMinMLN} are satisfied for some $\canparamwith$, if and only if $\meanparamwith$ is on the interior of a cube face of $\hlnmeanset$.
\end{lemma}
...
%\sect{Alternating Algorithms to Approximate the Backward Map}
...
%%
%The cases $\hypercoreat{\headvariableof{\selindex}=1}=0$, respectively $\hypercoreat{\headvariableof{\selindex}=1}=0$ only appear for nontrivial formulas when the distribution is not positive.
...
%            \State Compute
%            \[ \hypercoreat{\headvariableof{\selindex}}
%            \algdefsymbol \contractionof{\bencodingof{\enumformula}}{\headvariableof{\selindex}} \]
%            \State Set
%            \begin{align*}
...
\begin{theorem}
    ...
    \begin{align*}
        \forwardmapwrtof{\mlnmintermsymbol}{(\variableset,\headindexof{\variableset},\canparam)}
        = \normalizationof{\bencodingofat{\mintermformulaset}{\headvariableof{\mlnmintermsymbol},\shortcatvariables},
            \tnetofat{(\variableset,\headindexof{\variableset},\canparam)}{\headvariableof{\mlnmintermsymbol}},\identityat{\shortcatvariables,\selvariableof{[\atomorder]}}}{\selvariableof{[\atomorder]}}
    \end{align*}
    and
    \begin{align*}
        \forwardmapwrtof{\mlnmaxtermsymbol}{(\variableset,\headindexof{\variableset},\canparam)}
        = \onesat{\selvariableof{[\atomorder]}} - \normalizationof{\bencodingofat{\maxtermformulaset}{\headvariableof{\mlnmaxtermsymbol},\shortcatvariables},
            \tnetofat{(\variableset,\headindexof{\variableset},\canparam)}{\headvariableof{\mlnmaxtermsymbol}},\identityat{\shortcatvariables,\selvariableof{[\atomorder]}}}{\selvariableof{[\atomorder]}} \, .
    \end{align*}
    ...
    \begin{align*}
        ...
        = \Big(\Big\{ \selindex \wcols \selindexin \ncond \meanparamat{\indexedselvariable}=0 \Big\},1_{\variableset},-\lnof{\meanparamat{\selvariableof{[\atomorder]}}}\Big)
    \end{align*}
    (we set here $\lnof{0}=0$) is a backward map for the minterm statistic.
...
Any probability distribution can thus be represented by a minterm, respectively maxterm, \HybridLogicNetwork{}. %when we choose $\canparam=\lnof{\probtensor}$, respectively by maxterms when we choose $\canparam=\ones-\lnof{\probtensor}$.
...
%
%\begin{theorem} % TO BE DROPPED! Generalized to HLN
