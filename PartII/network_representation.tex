...
%        = \{\selindex\wcols\selindexin\ncond\kcorewith\in\{\fbasisat{\headvariableof{\selindex}},\tbasisat{\headvariableof{\selindex}}\}\}
    ...
    \begin{align*}
        \probofat{\formulaset,\{\kcoreof{\selindex}\wcols\selindexin\}}{\shortcatvariables}
        = \normalizationof{\mpformula}{\shortcatvariables} \, .
    \end{align*}
%
%
%    coincides with the uniform distribution % $\normalizationof{\mpformula}{\shortcatvariables}$, where
%%    \begin{align*}
%%        \mpfo
%%    \end{align*}
...
%    \begin{align*}
%        \probwith \in \expfamilyof{\mlnstat,\mpformula} \, ,
%    \end{align*}
%    where
%    \begin{align*}
%        \mpformula \coloneqq
...
%    %for any $\meanparamwith\in\{0,0.5,1\}^{\seldim}$ the formula
    ...
    \begin{align*}
        ...
        = \bigcup_{\variableset\subset[\seldim]} \bigcup_{\headindexof{\variableset}\in\bigtimes_{\selindex\in\variableset}[2]\wcols\contraction{\mpformula}>0} \expfamilyof{\mlnstat,\mpformula} \, ,
...
%    \end{align*}
    ...
    \begin{align*}
        ...
        &= \mpformulawith \, .
    \end{align*}
    We notice, that $\mpformulawith$ is satisfiable, since $\probwith$ is representable with respect to the base measure $\mpformulawith$.
    ...
    \begin{align*}
        \elrealizabledistsof{\mlnstat}
        \subset \bigcup_{\variableset\subset[\seldim]} \bigcup_{\headindexof{\variableset}\in\bigtimes_{\selindex\in\variableset}[2]\wcols\contraction{\mpformula}>0} \expfamilyof{\mlnstat,\mpformula} \, ,
    \end{align*}

    "$\supset$":\\
    Conversely, given an arbitrary pair $(\variableset,\headindexof{\variableset})$ we choose an arbitary element
    \begin{align*}
        \probofat{(\mlnstat,\canparam,\mpformula)}{\shortcatvariables} \in \expfamilyof{\mlnstat,\mpformula} \, .
    \end{align*}
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    Then we have $\probofat{(\mlnstat,\canparam,\mpformula)}{\shortcatvariables}\in\elrealizabledistsof{\mlnstat}$ since
    \begin{align*}
        \probwith =
...
\begin{theorem}
    Given a boolean statistic $\formulaset$, a basemeasure $\basemeasurewith$ and let $\meanparamwith$ be a boolean vector.
    Then the following are equivalent:
    \begin{itemize}
        \item[(i)] The set $\{\meanparamwith\}$ is a vertex of $\hlnmeanset$.
        \item[(ii)] The formula
...
%    \end{align*}
%    Finally, this is equivalent to the existence of a model of $\vertexformula$, i.e. the satisfiability of $\vertexformula$.
%    %ii$\Rightarrow$i \\
\end{proof}
