...
\begin{lemma}\label{lem:localHardLimit}
...
We exploit this characterization to show, that the vertices of $\hlnmeanset$ are exactly those reproducable by Hard Logic Networks for which all activation cores are either $\tbasisat{\headvariableof{\selindex}}$ or $\fbasisat{\headvariableof{\selindex}}$.
...
% True?
Since the by inclusion partially ordered set of faces is a graded lattice (see Theorem~2.7 in \cite{ziegler_lectures_2013}), the faces are ranked by the affine dimension of the convex hull. % the number of included vertices.
...
\end{theorem}
\begin{proof}
	\red{This is a special case of face measure to $\meanparam$.
	When mean param in 0/1, then the corresponding face is contained in the cube face.
	}
...
We can use this insight, to refine base measures by face base measures (see \defref{def:faceMeasure}), which has been shown in more generality in \theref{the:faceToArgmax}.
