...
We now extract from a Hybrid Logic Network, defined as a distribution computable by $\mlnstat$ and $\elgraph$, a \HardLogicNetwork{} and a \MarkovLogicNetwork{}, which contraction builds the Hybrid Logic Network.
...
\end{theorem}
\begin{proof}
    This follows from Theorem~\ref{the:factorReduction} on the representation of \HybridLogicNetworks{} as exponential distributions in \theref{the:hybridNetworkRepresentation}.
\end{proof}
...
\end{theorem}
\begin{proof}
    \theref{the:faceMeasureHardLogicNetworks} implies that the normalization of the face measure $\basemeasureof{\sstat,\facecondset}$ to $\genfaceset$ is a \HardLogicNetwork{} and in $\elrealizabledistsof{\formulaset}$.
...
\begin{example}[Minterm formulas]
    ...
    The faces of the standard simplex are cartesian products of $\bigtimes_{\selindex\in\variableset}\{0\}$ with standard simplices.
    We store them by the tuple $\cubefaceparam$, where $\headindexof{\selindex}=0$ for $\selindex\in\variableset$.
...
With the findings of the above sections, we can not characterize the set of mean parameters realizable by \HybridLogicNetworks{}.
