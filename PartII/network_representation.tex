...
\begin{theorem}[Relational Encodings for Markov Logic Networks]\label{the:mlnTensorRep}
	...
	\begin{align*}
		...
		= \begin{bmatrix} 1 \\
		 ...
		 \end{bmatrix}[\catvariableof{\formulaof{\selindex}}] \, .
	\end{align*}
\end{theorem}
\begin{proof}
...
% Discussion
While the statistics computing cores in the relational encoding are shared to compute the soft and the hard logic formulas, their head cores differ.
While probabilistic soft formulas get head cores (see Theorem~\ref{the:mlnTensorRep})
\begin{align*}
	\headcoreofat{\exformula}{\catvariableof{\exformula}} 
	= \begin{bmatrix} 1 \\
		 \expof{\canparamat{\indexedselvariableof{}}} 
		 \end{bmatrix}[\catvariableof{\formulaof{\selindex}}] \,
\end{align*}
...
\begin{align*}
	...
	= \begin{bmatrix} 0 \\
	 ...
	 \end{bmatrix}[\catvariableof{\formulaof{\selindex}}]  \, .
\end{align*}
As shown in Section~\ref{sec:hardLogicLimit}, the soft head cores converge to these hard head cores in the limit of large parameters, when imposing a local normation.
%
We further notice, that the probabilistic head cores are trivial tensors if and only if the corresponding parameter coordinate vanishes.
...
Deciding probabilistic entailment (see Definition~\ref{def:probEntailment}) with respect to Hybrid Logic Networks can be reduced to 
...
More generic correlations can be captured by a logical connective, calculated by a relational encoding and activated by a head core.}
...
% CSP
A more general class of problems, which have natural representations by Hard Logic Networks are Constraint Satisfaction Problems (see Chapter~5 in \cite{russel_artificial_2021}).
