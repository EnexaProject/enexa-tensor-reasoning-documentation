...
%\MarkovLogicNetworks{} are probability functions of truth assignments to logical functions.
...
\begin{definition}[\MarkovLogicNetwork]
    ...
    \begin{align*}
        \mlnstat \defcols\atomstates \rightarrow \bigtimes_{\exformulain}[2] \subset \rr^{\cardof{\formulaset}}
    \end{align*}
...
\begin{theorem}[Basis encodings for \MarkovLogicNetworks{}]
    ...
    \begin{align*}
        ...
        \normalizationof{\{\enumformulaccwith \wcols \selindexin \} \cup \{\enumformulaacwith \wcols \selindexin \}
        }{\shortcatvariables}
    \end{align*}
...
\theref{the:mlnTensorRep} provides a decomposition of \MarkovLogicNetworks{} by a tensor network of computation cores $\bencodingof{\enumformula}$ and accompanying activation cores $\enumformulaac$.
...
$\tilde{\formula}_{\selindex} \defcols\bigtimes_{} \rightarrow [2]$
with
\begin{align*}
...
%	The energy of \MarkovLogicNetworks{} is the contraction
...
%We here assumed, that the conjunction $\kb$ of the formulas in $\formulaset$ is satisfiable, and showed, that the \MarkovLogicNetwork{} converges in the limit of large weights to the uniform distribution of the models of $\kb$.
...
\begin{definition}[\HardLogicNetwork{}]
    ...
    Given a boolean statistic $\mlnstat$, a subset $\hardlegset\subset[\seldim]$ and a tuple $\headindexof{\hardlegset}\in\bigtimes_{\selindex\in\hardlegset}[2]$ the \HardLogicNetwork{} is the distribution
    \begin{align*}
        \probofat{\mlnstat,\hardparam}{\shortcatvariables}
        = \normalizationof{\mlnstatccwith,\kcoreofat{\hardparam}{\headvariables}}{\shortcatvariables}
    \end{align*}
    where $\kcoreofat{\hardparam}{\headvariables}=\bigotimes_{\selindexin}\kcoreofat{\selindex}{\headvariableof{\selindex}}$ and for $\selindexin$
    \begin{align*}
...
%    and for $\selindexin$ boolean vectors $\kcoreofat{\selindex}{\headvariableof{\selindex}}$ the \HardLogicNetwork{} is the distribution
%    \begin{align*}
...
%	The set of \HardLogicNetworks{} is defined as
%	\begin{align*}
...
\begin{theorem}\label{the:hlnFormulaCharacterization}
    For a boolean statistics $\mlnstat$ and parameters $\hardparam$ the \HardLogicNetwork{} exists, if and only if the formula
    \begin{align*}
        \hlnformula =
...
\begin{proof}
    ...
    \begin{align*}
        ...
        = \begin{cases}
              \enumformulawith &  \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\tbasisat{\headvariableof{\selindex}} \\% \andspace \headindexof{\selindex}=1 \\
              \lnot\enumformulawith & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\fbasisat{\headvariableof{\selindex}} \\
              \onesat{\shortcatvariables} & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\onesat{\headvariableof{\selindex}}
        \end{cases}
    \end{align*}
    With the definition of $\kcoreofat{\hardparam}{\headvariables}$ in \defref{def:hardLogicNetwork} we get
    \begin{align*}
        &\contractionof{\bencodingofat{\mlnstat}{\headvariables,\shortcatvariables},\kcoreofat{\hardparam}{\headvariables}}{\shortcatvariables}
        & \quad = \contractionof{
...
%We notice, that from the assumption that the normalization exists, each activation core needs to be non-vanishing and thus obeys
%\begin{align*}
...
%    we build the set
%    \begin{align*}
%        \hardlegset = \{\selindex\wcols\selindexin\ncond\kcorewith\in\{\fbasisat{\headvariableof{\selindex}},\tbasisat{\headvariableof{\selindex}}\}\}
%    \end{align*}
%    and the tuple $\headindexof{\hardlegset}\in\bigtimes_{\selindex\in\hardlegset}[2]$ by coordinates
%    \begin{align*}
%        \headindexof{\selindex}
%        = \begin{cases}
%              0 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}} = \fbasisat{\headvariableof{\selindex}} \\
%              1 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}} = \tbasisat{\headvariableof{\selindex}} \\
%        \end{cases} \, .
...
%\subsect{Tensor Network Representation}
%
%\HardLogicNetwork{} coincide with Knowledge Bases and are thus representable by contractions of formulas (which can be interpreted as a hybrid calculus scheme, see \secref{sec:hybridCalculus}).
%
%
...
%	and
%		\[ \kbat{\shortcatvariables} = \contractionof{\{\formulaccwith \wcols \formula\in\formulaset\} \cup \{\tbasisat{\formulavar} \wcols \formula\in\formulaset\} }{\shortcatvariables} \, .  \]
%\end{theorem}
%\begin{proof}
...
%
%\begin{remark}{$\land$ symmetry does not generalize to \MarkovLogicNetworks{}} % Strange -> Drop?
%	% Comparison to Markov Logic
%	In \MarkovLogicNetworks{}, similar decompositions are not possible.
...
\MarkovLogicNetworks{} are by definition positive distributions and are represented by positive activation tensors in $\elrealizabledistsof{\mlnstat}$ (see \figref{fig:elementaryComputableSketch}).
...
Towards defining \HybridLogicNetworks{}, we represent any distribution computable by $\mlnstat$ and $\elgraph$, as a normalization of a \HardLogicNetwork{} and a \MarkovLogicNetwork{}.

\begin{theorem}
    \label{the:hybridNetworkRepresentation}
    Any distribution $\probwith\in\elrealizabledistsof{\mlnstat}$ is a normalized contraction of a \HardLogicNetwork{} and a \MarkovLogicNetwork{}, that is there are for $\selindexin$ boolean cores $\kcoreofat{\selindex}{\headvariableof{\selindex}}$ and a weight vector $\canparamat{\selvariable}$ such that
    \begin{align*}
        \probwith =
        \normalizationof{
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \begin{cases}
            0 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}\neq\onesat{\headvariableof{\selindex}} \\
...
\begin{definition}[\HybridLogicNetwork{}]
    ...
    Given a boolean statistic $\mlnstat$ we call any element of $\elrealizabledistsof{\mlnstat}$ a \HybridLogicNetwork{} (HLN).
    For a subset $\hardlegset\subset[\seldim]$, a tuple $\headindexof{\hardlegset}\in\bigtimes_{\selindex\in\hardlegset}[2]$ and $\canparamwithin$ we denote
    \begin{align*}
        \hlnwith
        = \normalizationof{\mlnstatccwith,\actcoreofat{\canparam}{\headvariables},\kcoreofat{\hardparam}{\headvariables}}{\shortcatvariables} \, .
    \end{align*}
\end{definition}
...
\begin{theorem}
    Given a boolean statistic $\mlnstat$, we build for any $\hardlegset\subset[\seldim]$ and $\headindexof{\hardlegset}\in\bigtimes_{\selindex\in\hardlegset}[2]$ the formula
    ...
    \begin{align*}
        ...
        = \bigcup_{\hardlegset\subset[\seldim]} \,\, \bigcup_{\headindexof{\hardlegset}\in\bigtimes_{\selindex\in\hardlegset}[2]\wcols\contraction{\hlnformula}>0} \expfamilyof{\mlnstat,\hlnformula} \, .
    \end{align*}
\end{theorem}
\begin{proof}
    \theref{the:hybridNetworkRepresentation} implies that $\probwith\in\elrealizabledistsof{\mlnstat}$ is equivalent to the existence of parameters $\hybridparam$ with $\probwith=\hlnwith$.
    We further have that $\hlnwith$ is the member of the exponential family $\expfamilyof{\mlnstat,\hlnformula}$ with canonical parameter $\canparam$, since
    \begin{align*}
        \hlnwith
        &= \normalizationof{\mlnstatccwith,\actcoreofat{\canparam}{\headvariables},\kcoreofat{\hardparam}{\headvariables}}{\shortcatvariables} \\
        &= \normalizationof{\mlnstatccwith,\actcoreofat{\canparam}{\headvariables},\contractionof{\mlnstatccwith,\kcoreofat{\hardparam}{\headvariables}}{\shortcatvariables}}{\shortcatvariables} \\
        &= \normalizationof{\mlnstatccwith,\actcoreofat{\canparam}{\headvariables},\hlnformulawith}{\shortcatvariables} \, ,
    \end{align*}
    where we used \theref{the:hlnFormulaCharacterization} in the last equation.
    We therefore have $\probwith\in\elrealizabledistsof{\mlnstat}$ if and only if there is a tuple $\hardparam$ with $\probwith\in\expfamilyof{\mlnstat,\hlnformula}$.
%
%
...
%    \end{align*}
%    and a boolean tuple $\headindexof{\hardlegset}$ by
%    \begin{align*}
%        \headindexof{\selindex} =
%        \begin{cases}
%            0 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\fbasisat{\headvariableof{\selindex}} \\
...
%%        \begin{cases}
%%            0 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\fbasisat{\headvariableof{\selindex}} \\
%%            0.5 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\onesat{\headvariableof{\selindex}} \\
%%            1 & \ifspace \kcoreofat{\selindex}{\headvariableof{\selindex}}=\tbasisat{\headvariableof{\selindex}} \\
%%        \end{cases}
%%    \end{align*}
%    For the constructed pair $(\hardlegset,\headindexof{\hardlegset})$ it holds that
%    \begin{align*}
...
%    \begin{align*}
%        \elrealizabledistsof{\mlnstat}
%        \subset \bigcup_{\hardlegset\subset[\seldim]} \bigcup_{\headindexof{\hardlegset}\in\bigtimes_{\selindex\in\hardlegset}[2]\wcols\contraction{\hlnformula}>0} \expfamilyof{\mlnstat,\hlnformula} \, ,
%    \end{align*}
%
%    "$\supset$":\\
%    Conversely, given an arbitrary pair $(\hardlegset,\headindexof{\hardlegset})$ we choose an arbitary element
%    \begin{align*}
...
%        \begin{cases}
%            \onesat{\headvariableof{\selindex}} & \ifspace \selindex\notin\hardlegset \\
%            \tbasisat{\headvariableof{\selindex}} & \ifspace \selindex\in\hardlegset \quad \text{and} \quad \headindexof{\selindex}=1 \\
%            \fbasisat{\headvariableof{\selindex}} & \ifspace \selindex\in\hardlegset \quad \text{and} \quad \headindexof{\selindex}=0 \\
%        \end{cases}
%    \end{align*}
...
\begin{theorem}
    ...
    Let $\hlnwith$ be a Hybrid Logic Network.
...
\begin{theorem}%\label{the:hlnRepRedundancy}
    If for a Hybrid Logic Network $\hlnwith$ and any formula $\enumformula\in\formulaset$ we have
    \begin{align*}
        \hlnformula\models\enumformula  \quad \text{or} \quad \hlnformula\models\lnot\enumformula
    \end{align*}
    then
    \begin{align*}
        \hlnwith = \probofat{\mlnstat/\{\enumformula\},(\hardlegset/\{\selindex\},\headindexof{\hardlegset/\{\selindex\}},\tilde{\canparam})}{\shortcatvariables}
    \end{align*}
%    \[ \probofat{(\formulaset/\{\exformula\},\tilde{\canparam},\hlnformula)}{\shortcatvariables} =  \probofat{(\formulaset,\canparam,\hlnformula)}{\shortcatvariables}  \, , \]
    where $\tilde{\canparam}$ denotes the tensor $\canparam$, where the coordinate to $\enumformula$ is dropped. %, if $\exformula\in\softformulaset$.
\end{theorem}
\begin{proof}
...
%    \[ \basemeasure = \greaterof{\probat{\shortcatvariables}} \]
%    and a parameter tensor
%    \begin{align*}
%        \canparamat{\selvariableof{[\catorder]}=\shortcatindices}
%        = \begin{cases}
%              0 & \ifspace \probat{\indexedshortcatvariables} = 0  \\
...
\end{align*}
In \figref{fig:ActivatedHeads} is an example of a \HybridLogicNetwork{}, which computation network is represented using \decompositionSparsity{}.
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \end{align*}
        is satisfiable.
        \item[(iii)] The \HardLogicNetwork{} $\probofat{\mlnstat,([\seldim],\meanparam)}{\shortcatvariables}$ exists. % There is a \HardLogicNetwork{} $\probwith\in\hlnsetof{\mlnstat}$ such that
%        \begin{align*}
...
%                         \fbasisat{\headvariableof{\selindex}} & \ifspace \meanparamat{\indexedselvariable}=0 \\
%                         \tbasisat{\headvariableof{\selindex}} & \ifspace \meanparamat{\indexedselvariable}=1 \\
%        \end{cases}
%    \end{align*}
%    and notice that
%    \begin{align*}
...
%    \end{align*}
    ...
    (iii)$\Rightarrow$(i):
    The mean parameter is reproduced by $\probofat{\mlnstat,([\seldim],\meanparam)}{\shortcatvariables}$ and therefore $\meanparamwith\in\hlnmeanset$.
    Since $\meanparamwith$ is further boolean, \theref{the:vertexByHardLogicNetworks} implies that $\{\meanparamwith\}$ is a vertex.
\end{proof}

% \HardLogicNetwork{} representation -> Another theorem?
%Let us notice, that the normalization of $\vertexformula$ is a \HardLogicNetwork{} using the statistic $\formulaset$ and the base measure $\basemeasure$.
%Any model of this formula is furthermore mapped by the statistics encoding onto the corresponding vertex $\{\meanparamwith\}$ and the mean parameter of the \HardLogicNetwork{} is the unique vertex element $\meanparamwith$.
%We will in \charef{cha:networkReasoning} show, that among the distributions having the vertex as mean parameter, the corresponding \HardLogicNetwork{} is the one with maximal entropy.
...
% \HardLogicNetwork{}
\theref{the:faceMeasureHardLogicNetworks} characterizes the faces, which measures are reproducable by \HardLogicNetworks{}.
...
\red{When the normalized face measure to a face is not reproduceable by a \HardLogicNetwork{}, it does not lie in $\realizabledistsof{\sstat,\elgraph,\basemeasure}$.
...
\subsect{Mean parameters reproducable by \HybridLogicNetworks{}}\label{sec:HLNrepMean} % Generalize to any mean parameter reproduction by \HybridLogicNetworks{}
...
Let us find conditions on a mean parameter $\meanparamwith$ for the existence of a Hybrid Logic Network reproducing it. %$\meanparamwith$.
...
\begin{example}[Atomic formulas]\label{exa:atomicFormulasHypercube}
...
\begin{definition}\label{def:elementaryRealizableMeanParams}
...
\begin{example}[Minterm formulas]\label{exa:mintermHLNSet}
...
%\end{theorem}
%\begin{proof}
%    The mean parameters to the effective interior of any face $\hlnfaceset$ of $\hlnsetof{\mlnstat}$ are reproducable by $\hlnsetof{\mlnstat}$, if and only if the normalization of the corresponding face measure is in $\hlnsetof{\mlnstat}$.
%    This is exactly the case, when the face is the intersection $\hlnfaceset=\hlnmeanset\cap\cubeface$ for a face $\cubeface$ of $\fullparcube$.
...
% Activation vectors of \HardLogicNetworks{} are on the other side basis+, so they might be more efficiently representable
