...
\begin{theorem}
    Given a boolean statistic $\mlnstat$, we build for any $\meanparamwith\in\{0,0.5,1\}^{\seldim}$ the formula
    \begin{align*}
        \mpformula \coloneqq
        \left(\bigwedge_{\selindex\wcols\indexedmeanparam=1} \enumformulaat{\shortcatvariables}\right)
        \land
        \left(\bigwedge_{\selindex\wcols\indexedmeanparam=0} \lnot\enumformulaat{\shortcatvariables}\right) \, .
    \end{align*}
    Then we have
    \begin{align*}
        \elrealizabledistsof{\mlnstat}
        = \bigcup_{\meanparamwith\in\{0,0.5,1\}^{\seldim}\wcols\contraction{\formulaof{\mlnstat,\meanparamwith}}>0} \expfamilyof{\mlnstat,\mpformula} \, ,
    \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \land
        \left(\bigwedge_{\selindex\wcols\indexedmeanparam=0} \lnot\enumformulaat{\shortcatvariables}\right)
        = \formulaofat{\mlnstat,\meanparamwith}{\shortcatvariables} \, .
    \end{align*}
    We notice, that $\formulaofat{\mlnstat,\meanparamwith}{\shortcatvariables}$ is satisfiable, since $\probwith$ is representable with respect to the base measure $\formulaofat{\mlnstat,\meanparam}{\shortcatvariables}$.
    It follows that $\probwith$ is an element of the exponential family $\expfamilyof{\mlnstat,\mpformula}$.

    "$\subset$":\\
    Conversely, given an arbitrary $\meanparamwith\in\{0,0.5,1\}^{\seldim}$ we have, provided that $\mpformula$ is satisfiable
    \begin{align*}
        \expfamilyof{\mlnstat,\mpformula}
    \end{align*}
\end{proof}
...
For an example of a tensor network representation of a hybrid logic netaork see \figref{fig:ActivatedHeads}.
...
\begin{theorem}
    ...
    Let $\probofat{(\mlnstat,\canparam,\mpformula)}{\shortcatvariables}$ be a Hybrid Logic Network.
    Given a query formula $\exformula$ we have that
    \begin{align*}
        \probtensorof{\mlnstat,\canparam,\mpformula} \models \exformula
    \end{align*}
    if and only if
    \[ \mpformula \models \exformula \, . \]
\end{theorem}
\begin{proof}
...
\begin{theorem}%\label{the:hlnRepRedundancy}
    If for a Hybrid Logic Network $\probof{(\formulaset,\canparam,\mpformula)}$ and any formula $\exformula\in\formulaset$ we have
    \[ \mpformula \models \exformula \, \quad \text{or} \quad \mpformula \models \lnot\exformula \]
    then
    \[ \probofat{(\formulaset/\{\exformula\},\tilde{\canparam},\mpformula)}{\shortcatvariables} =  \probofat{(\formulaset,\canparam,\mpformula)}{\shortcatvariables}  \, , \]
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item The formula
        \begin{align*}
            \mpformula = \bigwedge_{\selindexin} \lnot^{(1-\meanparamat{\indexedselvariable})} \enumformulaat{\shortcatvariables}
        \end{align*}
        is satisfiable.
    \end{itemize}
...
\begin{proof}
    %i$\Rightarrow$ii \\
    If and only if $\{\meanparamwith\}$ is a vertex of $\hlnmeanset$, then there is $\shortcatindices\in\atomstates$ such that % in particular $\meanparamwith\in$.
    \begin{align*}
    ...
    \end{align*}
    Finally, this is equivalent to the existence of a model of $\mpformula$, i.e. the satisfiability of $\mpformula$.
    %ii$\Rightarrow$i \\
\end{proof}

% Hard Logic Network representation -> Another theorem?
Let us notice, that the normalization of $\mpformula$ is a Hard Logic Network using the statistic $\formulaset$ and the base measure $\basemeasure$.
...
\begin{theorem}
    ...
    \begin{itemize}
        \item[(i)] There is a face $\cubeface$ of $\fullparcube$ such that $\genfaceset=\hlnmeanset\cup\cubeface$.
...
(i)
    $\Rightarrow$(ii):
    Given the face $\cubeface$ such that $\genfaceset=\hlnmeanset\cup\cubeface$, we construct a boolean and basis+ elementary tensor (see \charef{cha:sparseRepresentation}), such that
    \begin{align*}
...
\begin{theorem}
    ...
    \begin{align*}
        \hlnfacemeasure
        = \bigvee_{\meanparam\in\genfaceset\cup\boundaryparcube} \mpformula
...
\begin{proof}
    ...
    \end{align*}
    We notice, that for each mean parameter $\meanparam\in\genfacesetof{\facecondset}\cup\imageof{\sstatencoding}$ the boolean tensors
    \begin{align*}
        \contractionof{\mlnstatccwith,\onehotmapofat{\headindex^{\meanparam}_{[\seldim]}}{\headvariables}}{\shortcatvariables}
    \end{align*}
    coincide with $\mpformula$.
    Further, the formulas $\mpformula$ have disjoint model sets and their disjunction is therefore represented by their sum.
    We therefore have
    \begin{align*}
         \hlnfacemeasureat{\shortcatvariables}
            &=\sum_{\meanparam\in\genfacesetof{\facecondset}\cup\imageof{\sstatencoding}} \mpformulaat{\shortcatvariables} \\
            &=\bigvee_{\meanparam\in\genfaceset\cup\boundaryparcube}
...
%    By representing the contraction with $\kcoreofat{\facecondset}{\sstatheadvariables}$.
 %   Any basis tensors is a disjoint propositional formulas, the vertex formula $\mpformula$ and their sum is thus represented by the disjunction.
\end{proof}


\subsect{Mean parameters reproducable by Hybrid Logic Networks} % Generalize to any mean parameter reproduction by hybrid logic networks
...
\begin{theorem}
    ...
    Let $\mlnstat$ be a boolean statistics and $\canparamwithin$.
    In the limit $\invtemp\rightarrow\infty$ the distribution $\probofat{\formulaset,\invtemp\cdot\canparam}{\shortcatvariables}$ converges coordinatewise to the normalized face measure $\basemeasureofat{\formulaset,\canparam}{\shortcatvariables}$, that is for any $\shortcatindices\in\facstates$ we have
    \begin{align*}
        \probofat{\formulaset,\invtemp\cdot\canparam}{\indexedshortcatvariables}
        \rightarrow \frac{1}{\contraction{\basemeasureofat{\formulaset,\canparam}{\shortcatvariables}}} \cdot \basemeasureofat{\formulaset,\canparam}{\indexedshortcatvariables} \, .
    \end{align*}
\end{theorem}
\begin{proof}
...
\begin{theorem}
    ...
    \end{align*}
    is satisfiable, then $\probofat{\formulaset,\invtemp\cdot\canparam}{\shortcatvariables}$ converges for $\invtemp\rightarrow\infty$ coordinatewise to the normalized uniform distribution among the models of $\exformula^{\canparam}$.
\end{theorem}
\begin{proof}
    If $\exformula^{\canparam}$ is satisfiable, then the face measure $\basemeasureofat{\formulaset,\canparam}{\shortcatvariables}$ is the uniform distribution among the models of $\exformula^{\canparam}$.
    The claim follows from \theref{the:limitingDistribution}.
\end{proof}

We notice, that $\exformula^{\canparam}$ is computable with elementary activation cores, that is for
\begin{align*}
...
we have
\begin{align*}
    \formulaofat{\canparam}{\shortcatvariables} =
    \normalizationof{\bencodingofat{\mlnstat}{\sstatheadvariables,\shortcatvariables}\actcoreat{\headvariables}}{\shortcatvariables}
\end{align*}
...
\begin{figure}[t]
    ...
    \textcolor{\concolor}{Face centers} $\meanparamof{1}$ (in particular the vertices $\hlnmeanset\cup\boundaryparcube$) are reproducable by a Hard Logic Network given $\mlnstat$.
    Mean points on non-vertex faces outside the interior $\meanparamof{2}\in\hlnmeanset/(\interiorof{\hlnmeanset}\cup\boundaryparcube)$ are reproducable by Hybrid Logic Networks with statistic $\mlnstat$ and refined base measure $\secbasemeasure$.
...
\begin{proof}
    ...
    This is exactly the case, when the face is the intersection $\genfaceset=\hlnmeanset\cup\cubeface$ for a face $\cubeface$ of $\fullparcube$.
