...
\begin{proof}
    "(i)$\Rightarrow$(ii)": \\
    Given the face $\cubeface$ such that $\genfaceset=\genmeanset\cup\cubeface$, we construct a boolean and basis+ elementary tensor (see \charef{cha:sparseRepresentation}), such that
    \begin{align*}
...
The core $\kcoreofat{\facecondset}{\headvariableof{[\seldim]}}$ has then $\cpformat$ rank of at least two.}
...
\end{theorem}
\begin{proof}
    This follows from \theref{the:hlnInteriorCharacterization} and \theref{lem:minimalContainingFace}.
\end{proof}
...
\end{theorem}
\begin{proof}
    This follows from a characterization of the face measure and \theref{the:limitingDistribution}.
\end{proof}
...
\red{In \figref{fig:reproducableSketch} we classify the distributions by their mean parameters in $\hlnmeanset$.}
...
%	Any extreme point $\meanparamof{1}\in\hlnmeanset\cap\boundaryparcube$ is reproducable by a Hard Logic Network,
%	while a non-extreme boundary point $\meanparamof{2}\in\hlnmeanset/\boundaryparcube$ is realizable by a Hybrid Logic Network.
...
%	Contractions with the computation network are effectively constraining only a subset of statistic coordinates (use the ones-trivial-property of basis encodings).
...
%% As example $\exaref{exa:vertexMeasure}
