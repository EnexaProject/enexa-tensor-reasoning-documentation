...
\begin{document}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.35, thick]
            ...
            \node[anchor=center] (text) at (-2,0) {\small $a)$};

            \draw (-1,-5) rectangle (1, -7);
            \node[anchor=center] (text) at (0,-6) {\small $K^{A1}$};

            \draw[] (0,-3)--(0,-5)  node[midway,left] {\tiny $X_{A1}$};
            \draw[]  (3,-3) to[bend left = -20] (4.5,-5);
            \draw (-1,-1) rectangle (4, -3);
            \node[anchor=center] (text) at (1.5,-2) {\small ${A1} \oplus {A2}$};

            \draw (5,-1) rectangle (10, -3);
            \node[anchor=center] (text) at (7.5,-2) {\small $E \Rightarrow A2$};
            \draw (6,-3) to[bend left = 20] (4.5,-5);
            \draw[] (4.5,-5)--(4.5,-7) node[midway,right] {\tiny $X_{A2}$};
            \draw (3.5,-9) rectangle (5.5, -7);
            \node[anchor=center] (text) at (4.5,-8) {\small $K^{A2}$};

            \draw[] (9,-3)--(9,-5) node[midway,right] {\tiny $X_E$};

            \draw (8,-5) rectangle (10, -7);
            \node[anchor=center] (text) at (9,-6) {\small $K^{E}$};

            \draw[fill] (4.5,-5) circle (0.15cm);

            \begin{scope}
                [shift={(25,0)}]

                \node[anchor=center] (text) at (-6,0) {\small $b)$};
                ...
                % Edges
                \draw (A) -- (B) node [midway, left] {\tiny $K^{E}=e_1$};
                \draw (A) -- (B1) node [midway, right] {\tiny $K^{E}=e_0$};
                \draw (B) -- (C) node [midway, left] {\tiny $K^{A2}=e_1$};
                \draw (C) -- (D) node [midway, left] {\tiny $K^{A1}=e_1$};

                \draw (B1) -- (C1) node [midway, left] {\tiny $K^{A2}=e_1$};
                \draw (B1) -- (C2) node [midway, right] {\tiny $K^{A2}=e_0$};

                \draw (C1) -- (D1) node [midway, right] {\tiny $K^{A1}=e_0$};
                \draw (C2) -- (D2) node [midway, right] {\tiny $K^{A1}=e_1$};
