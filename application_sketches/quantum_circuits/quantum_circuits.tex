...
%        Given a probability distribution $\probtensor:\atomstates\rightarrow\rr$ (i.e. $\contraction{\probtensor}=1$ and $\zeros \prec \probtensor$) its amplitude encoding is
%        \begin{align*}
...
%    \begin{definition}[Ancilla Amplitude Encoding]
%        Given a non-negative tensor $\hypercoreat{\headvariables}$ its ancilla amplitude encoding is the unitary tensor
%        $\qcaencodingofat{\hypercore}{\avariableof{\insymbol},\avariableof{\outsymbol},\headvariableof{\insymbol,[\seldim]},\headvariableof{\outsymbol,[\seldim]}}$ which satisfies for each $\headindexof{[\seldim]}$
%        \begin{align*}
...
%    \end{definition}
   ...
   % The ancilla amplitude encoding is the pendant of the coordinate encoding of $\sqrt{\probtensor}$ (see Chapter Coordinate Calculus).
...
%    \end{lemma}
    ...
    The basis encoding circuit to each boolean monomial has a representation by a multiple controlled $\paulixsymbol$ gate and further pairs of $\paulixsymbol$ gates preparing the control state, see \figref{fig:qcbencodingPolynomial}.
...
%    \begin{lemma}
%        The amplitude encoding of $\probwith$ acting on the uniform state of $\shortcatvariables$ prepares a q-sample.
%    \end{lemma}
%    \begin{proof}
%        Follows directly from construction:
%        The uniform state is the action of Hadamard gates on the ground state $\onehotmapofat{0}{\shortcatvariables}$, which prepares the state $\normalizationof{\ones}{\catvariables}$.
%        The diagonal amplitude encoding transforms
%    \end{proof}
...
%    \begin{definition}[Augmented Amplitude Encoding]
...
%        We call a quantum state $\qstateofat{\probtensor}{\catvariables,\avariables}$ an augmented amplitude encoding, if
%        \begin{align*}
