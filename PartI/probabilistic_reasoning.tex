...
\begin{definition}[Shannon entropy]
	...
	\begin{center}
		\input{PartI/tikz_pics/probabilistic_reasoning/shannon_entropy.tex}
	\end{center}
\end{definition}
...
\begin{definition}[Cross entropy]\label{def:crossEntropy}
	...
	\begin{center}
		\input{PartI/tikz_pics/probabilistic_reasoning/cross_entropy.tex}
	\end{center}
\end{definition}
...
\begin{theorem}
	...
	%\end{align*}
	%and 
	by $\meanrepprob$ we denote a probability tensor reproducing the mean parameter $\meanparam$.
\end{theorem}
\begin{proof}
...
% Theorem out of this?
	For any statistics $\sstat$, which is minimal with respect to a base measure $\basemeasure$, the image $\imageof{\forwardmap}$ of the forward map is the interior of the convex polytope $\meansetof{\sstat}$.
...
For the practicle usage of this theorem, we need a characterization of the interior of $\meansetof{\sstat}$.
...
\begin{theorem}\label{the:meanPolytopeInteriorCharacterization}
	For any minimal statistics $\sstat$ and boolean base measure $\basemeasure$ we have for some $\meanparamat{\selvariable}$ that $\meanparamat{\selvariable}\in\meanset{\sstat,\basemeasure}$ if and only if there is a positive distribution with respect to $\basemeasure$ such that
		\[ \meanparamat{\selvariable} = \sbcontractionof{\probtensor,\sencsstat}{\selvariable} \, . \]
%	If $\meanparamat{\selvariable}$ is in an minimal exponential family with boolean base measure $\basemeasure$, then it is in the interior of $\meanset$ if and only if it is representable by a positive distribution with respect to $\basemeasure$.
\end{theorem}
\begin{proof} 
	\proofrightsymbol: 
		By Theorem~\ref{the:meanPolytopeInterior} we find a canonical parameter $\canparamat{\selvariable}$ such that
		\begin{align*}
	...
	\proofleftsymbol: % Orient on proof of Theorem~3.3 
		Since by assumption the statistics is minimal, the convex set $\meansetof{\sstat,\basemeasure}$ is full dimensional (see e.g. Appendix B in \cite{wainwright_graphical_2008}). 
		We thus use a well-known property for full-dimensional convex sets (see \cite{rockafellar_convex_1997,hiriart-urruty_convex_1993}), that $\meanparam\in\interiorof{\meansetof{\sstat,\basemeasure}}$ if for any non-vanishing vector $\vectorat{\selvariable}$ there is a  % citations from Wainwright - Appendix B	
		...
		\begin{align*}
			...
			= \contractionof{\probwith,(\contractionof{\sencsstatwith,\vectorat{\selvariable}})^2}{\shortcatvariables}
			 - \left(\contractionof{\probwith,\sencsstatwith,\vectorat{\selvariable}}{\shortcatvariables}\right)^2 \, . 
		\end{align*}
...
\subsection{Boundary of convex polytopes}

For mean parameters $\meanparamat{\selvariable}$ outside the interior of $\meansetof{\sstat,\basemeasure}$ we know by Theorem~\ref{the:meanPolytopeInteriorCharacterization}, that any distribution with mean parameter $\meanparamat{\selvariable}$ is not positive with respect to $\basemeasure$ and is therefore not in the exponential family.
...
%\end{lemma}
\begin{proof}
	We use the halfspace representation of Theorem~\ref{the:meanPolytopeHalfspaces} to show the first claim.
	By the above lemma, $\meanparamat{\selvariable}$ is not in the interior, therefore on a face and thus the $\canparam$ is found by one $a_i[\selvariable]$ of that face.
	
	This face is the convex hull of the extreme points of $\meanset$ corresponding with the elements of $\arbset$.
...
The parameters optimizing the likelihood, will be shown to coincide with the backward mapping evaluated on the expectation of the sufficient statistics (see Theorem~\ref{the:parEstToBackwardMap}).
