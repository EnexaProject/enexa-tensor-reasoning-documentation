...
More details on this scheme are provided in Chapter~\ref{cha:basisCalculus}, see \defref{def:functionRelationEncoding} therein.
...
Let us note that they are further contraction of the queries in \defref{def:queries} since by Theorem~\ref{the:splittingContractions}
\begin{align*}
...
For mean parameters $\meanparamat{\selvariable}$ outside the interior of $\genmeanset$ we know by Theorem~\ref{the:meanPolytopeInteriorCharacterization}, that any distribution with mean parameter $\meanparamat{\selvariable}$ is not positive with respect to $\basemeasure$ and is therefore not in the exponential family.
...
%
Theorem~\ref{the:maxEntMaxLikeDuality} states, that when the maximum entropy problem has a solution (i.e. $\genmean\in\genmeanset$), then the solution is in the exponential family to the statistic $\sstat$.
