...
The efficient retrieval of information stored in tensors has to exploit the available decomposition schemes.
To avoid the instantiation of the orginal tensor based on its decomposition, we directly define deductive reasoning schemes by contractions, which can be executed using the available decomposition.
...
%We have argued that one should avoid naive instantiation of these distributions based on storage of each coordinates.
%In the task of reasoning, we want to retrieve information encoded in the probability distribution.
%To derive an efficient approach one therefore needs to avoid instantiating the distribution in a coordinatewise manner in an intermediate step.
...
Conditional distributions can be similarly retrieved by two identity statistics.
The framework of \defref{def:queries} can be beyond that used for customized queries.
...
\begin{align*}
    ...
    = \sbcontraction{\probat{\headvariableof{\exfunction}},\idrestrictedto{\imageof{\exfunction}}{\headvariableof{\exfunction}} } \, .
\end{align*}
This will be shown in more detail in \charef{cha:basisCalculus} in \corref{cor:rhoToNormal}.
...
\subsect{Answering queries by energy contractions}
...
%\end{lemma}
%\begin{proof}
%	By splitting the contraction into terms to $\nodesa\cup\nodesb$. % and using \theref{the:energyContractionQueries}.
%\end{proof}




\sect{Sampling based on queries}
...
Naive methods, such as drawing a random number in $[0,1]$, adding iteratively the coordinates and stopping when the sum exceeds the random variables are infeasible because of the number of coordinates.
...
Gibbs can be implemented based on the energy tensor $\energytensor$ of the probability tensor, as follows form the \theref{the:energyContractionQueries}.
\red{This is in contrast with forward sampling, where we need to marginalize out variables, which can not be done without performing the exponential coordinate transform first.
The reason for this is that summations and exponentiations do not commute.
%sum over many coordinates of the exponentiated energy tensor, which amounts to the representation of the probability distribution as a tensor network using relational encodings.
}



\subsect{Simulated Annealing}
...
\subsect{Likelihood and Loss}

Given a datapoint $\datamapat{\datindex}$ consisting of the images of the data selecting map $\datamap$ (see \defref{def:dataMap}), the likelihood given a Markov Logic Network is denoted as
...
\subsect{Boundary of convex polytopes}
...
\subsect{Mode Search by annealing}
