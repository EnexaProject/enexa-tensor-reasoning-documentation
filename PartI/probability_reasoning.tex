...
\theref{the:maxEntropyInterior} implies, that the by the face measure refined base measure $\secbasemeasure$ is minimal for the maximum entropy problem, in the sense that the solving distribution is positive with respect to it and all feasible distributions have to be representable by it.
...
\begin{align*}
    \meanparam(\genfacesetof{\canparam}) = \contractionof{\sencsstatwith,\normalizationof{\basemeasureof{\sstat,\canparam}}{\shortcatvariables}}{\selvariable} \, .
\end{align*}
...
\begin{example}[Faces with normals parallel to one-hot encodings]
    ...
    We use basis encodings of the coordinates $\sstatcoordinateof{\selindex}$ of the statistic $\sstat$, with head variables $\catvariableof{\sstatcoordinateof{\selindex}}$ with dimension $\catdimof{\sstatcoordinateof{\selindex}}$ enumerating the image $\imageof{\sstatcoordinateof{\selindex}}\subset\rr$ in an ascending order.
    ...
    = \contractionof{\bencodingofat{\sstatcoordinateof{\selindex}}{\shortcatvariables,\catvariableof{\sstatcoordinateof{\selindex}}},
...
\begin{proof}
    ...
    We notice, that the normalization of any face base measure is realizable by $\realizabledistsof{\sstat,\maxgraph}$. %since the objective in the maximation problem in \defref{def:faceBaseMeasure} depends only on $\sstat$. ?
    ...
    \begin{align*}
        \secbasemeasure = \contractionof{
            \{\bencodingofat{\sstat}{\sstatheadvariables,\shortcatvariables}\} \cup
...
%The backward mapping to $\datameanat{\selvariable} = \contractionof{\empdistribution,\sencsstat}{\selvariable}$ is Maximum Likelihood estimation and the solution of the maximum entropy problem.
...
\begin{lemma}
    ...
    \begin{align*}
        \contraction{\actcoreof{\selindex}, \idrestrictedto{\imageof{\sstatcoordinateof{\selindex}}}, \hypercoreat{\selvariable_\sstat}}
        = \contraction{\actcoreof{\selindex}, \hypercoreat{\selvariable_\sstat}} \cdot \meanparamat{\indexedselvariable} \, .
    \end{align*}
\end{lemma}
\begin{proof}
    We have
    \begin{align*}
        \expdist = \frac{
            \contractionof{\actcoreof{\selindex}, \hypercore}{\shortcatvariables}
        }{
            \contraction{\actcoreof{\selindex}, \hypercore}
        }
    \end{align*}
    and
    \begin{align*}
        ...
        = \frac{
            \contractionof{\actcoreof{\selindex}, \idrestrictedto{\imageof{\sstatcoordinateof{\selindex}}}, \hypercore}{\shortcatvariables}
        }{
            \contraction{\actcoreof{\selindex}, \hypercore}
        } \, .
    \end{align*}
    Here we used
    \[ \sstatcoordinateof{\selindex} = \contractionof{\actcoreof{\selindex}, \idrestrictedto{\imageof{\sstatcoordinateof{\selindex}}}}{\shortcatvariables} \]
    and redundancies of copies of basis encodings.
    It follows that
    \begin{align*}
        \contraction{\expdist,\sstatcoordinateof{\selindex}} = \contraction{\empdistribution,\sstatcoordinateof{\selindex}}
    \end{align*}
    is equal to
    \begin{align*}
        \contraction{\actcoreof{\selindex}, \idrestrictedto{\imageof{\sstatcoordinateof{\selindex}}}, \hypercoreat{\catvariableof{\sstatcoordinateof{\selindex}}}}
        = \contraction{\actcoreof{\selindex},\hypercoreat{\catvariableof{\sstatcoordinateof{\selindex}}}} \cdot \meanparamat{\indexedselvariable} \, . & \qedhere
    \end{align*}
\end{proof}
    ...
    \begin{algorithmic}
        ...
        \While{Stopping criterion is not met}
            \For{$\selindexin$}
                ...
                \begin{align*}
                    \contraction{\actcoreof{\selindex},\idrestrictedto{\imageof{\sstatcoordinateof{\selindex}}},\hypercoreof{\selindex}}
                    = \contraction{\actcoreof{\selindex},\hypercoreof{\selindex}} \cdot \datameanat{\indexedselvariable} \, .
                \end{align*}
            \EndFor
        \EndWhile
