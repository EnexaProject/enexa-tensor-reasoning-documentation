...
\begin{theorem}
    ...
    \end{align*}
    Then the corresponding maximum entropy distribution is in $\realizabledistsof{\sstat,\graph,\basemeasure}$ if the sum of vertices
    \begin{align*}
        \kcoreofat{\facecondset}{\headvariableof{[\seldim]}}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \contractionof{\bencodingofat{\sstat}{\headvariableof{[\seldim]},\shortcatvariables},\kcoreofat{\facecondset}{\headvariableof{[\seldim]}}}{\shortcatvariables}
    \end{align*}
    Then for the tensor
    \begin{align*}
        \hypercoreat{\headvariableof{[\seldim]}}
        = \contractionof{
            \{\actcoreofat{\selindex,\canparamat{\indexedselvariable}}{\headvariableof{\selindex}}\wcols\selindexin\}
            \cup\{\kcoreofat{\facecondset}{\headvariableof{[\seldim]}}\}}{\headvariableof{[\seldim]}}
    \end{align*}
    we have
    \begin{align*}
        \probofat{\sstat,\canparam,\secbasemeasure}{\shortcatvariables}
        = \normalizationof{
            \bencodingofat{\sstat}{\headvariableof{[\seldim]},\shortcatvariables}, \hypercoreat{\headvariableof{[\seldim]}}, \basemeasurewith
        }{\shortcatvariables} \, .
    \end{align*}
...
\begin{proof}
    ...
    In this case we have $\kcoreofat{\varnothing}{\headvariableof{[\seldim]}}=\onesat{\headvariableof{[\seldim]}}$, which is an elementary tensor.
...
\begin{example}[Representation by the maximal graph]
    ...
    Maximum entropy distributions with respect to constraints $\meanparamat{\selvariable}=\contractionof{\probtensor,\sencsstat}{\selvariable}$ therefore always have the sufficient statistic $\sstat$.
...
\begin{proof}
    ...
    By \theref{lem:faceToCanparam} the maximum is taken at the face $\genfacesetof{\canparam}$.
