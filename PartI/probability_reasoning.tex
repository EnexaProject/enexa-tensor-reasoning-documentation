...
\begin{align*}
    \dualcumfunction(\meanparam) = \max_{\canparam\in\simpleparspace} \contraction{\meanparam,\canparam} - \cumfunctionof{\canparam} \, .
\end{align*}

\begin{lemma}\label{lem:gradientCumfunction}
    Let $\canparam\in\simpleparspace$ and $\meanparamwith$ the corresponding mean parameter.
...
\begin{theorem}[Variational forward map]
    Let $\sstat$ be a minimal statistic with respect to $\basemeasure$.
    Given $\canparam$, there is a unique $\meanparam$ with
    \begin{align*}
         \meanparam \in \argmax_{\meanparam\in\meanset} \contraction{\meanparam,\canparam} -  \dualcumfunction(\meanparam)
    \end{align*}
...
\begin{proof}
    ...
    \end{align*}
    The statement then follows from the first order condition, where we use the gradient \lemref{lem:gradientCumfunction}. % Do we need to assume minimal statistics?
\end{proof}
...
%We have at any $\canparam\in\simpleparspace$
%\begin{align*}
...
We provide further intuition o n the position of the mean parameter, when interpreting $\basemeasure$ and $\sstat$ as soft and hard constraint meachanisms on a distribution.
...
% Statistics encoding
We define the statistics encoding of states as a map
\begin{align*}
    \sstatencoding : \stateset \rightarrow \simpleparspace \quad, \quad
...
The choice of a base measure $\basemeasure$ can be regarded as hard structure of a distribution.
...
We define for each face the cone
\begin{align*}
    \maxcone = \{\canparam\wcols\canparam\in\simpleparspace\ncond\argmax_{\meanparam\in\genmeanset}\contraction{\canparam,\meanparam}=\genfaceset\}
\end{align*}

% Maybe earlier to avoid minimal statistics discussion
\begin{definition}
    We say, that a set $\arbset\subset\simpleparspace$ is effectively open, if $\arbset$ is contained in an affine subspace and open in that affine subspace.
\end{definition}
...
\lemref{lem:faceSetsToMaxCones} suggests that the partial order of faces by inclusion is mimiked by another partial order on the max cones.
...
\begin{proof}
    ...
    Further, since the max cones are a disjoint partition of $\simpleparspace$, $\zerosat{\selvariable}$ is not in any other max cone and thus $\genmaxconeof{\facecondset} \prec \genmaxconeof{\facecondsetof{\varnothing}}$ does not hold for any non-empty $\facecondset$.
...
\begin{figure}
    ...
    \end{center}
    \caption{
        Partition of the canonical parameters in $\simpleparspace$ into convex and effectively open maximum cones $\maxcone$ to faces $\facecondset$ (left).
...
\begin{lemma}\label{lem:hesseCumfunction}
    At any $\canparam\in\simpleparspace$ we have
    \begin{align*}
