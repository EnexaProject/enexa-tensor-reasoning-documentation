...
\begin{definition}
    ...
    \begin{align}
        \argmax_{\shortcatindices} \hypercoreat{\indexedshortcatvariables} \, .
    \end{align}
\end{definition}
...
\end{align*}
We further introduce the likelihood of $\probwith$
\begin{align*}
...
When this distribution is $\probwith$, then the probability of generating the dataset $\dataset$ by this scheme coincides with the likelihood.
...
We will therefore compute and compare the likelihood of distributions, which in general not coincide with the distribution generating the data.
...
The mean parameter coordinates are expectation queries to $\sstatcoordinateof{\selindex}$, by
