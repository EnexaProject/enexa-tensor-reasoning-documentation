...
\end{align}
With \lemref{lem:centropyMLE} we have established, that maximum likelihood estimation is the M-projection of the empirical distribution onto a set $\Gamma$.
...
\begin{align*}
    ...
    = \genmeansetargmax \sbcontraction{\meanparam,\canparam}  \, .
\end{align*}
For a more detailed discussion of this relation, see the proof of Theorem~8.1 in \cite{wainwright_graphical_2008}.
...
\begin{theorem}\label{the:meanFieldIProjection}
...
%	This follows from the fact, that the objective is the cross-entropy and the position of the maximum is invariant under substracting $\sentropyof{\probtensor}$.
    ...
    \end{align*}
    Since the last term is constant among $\probtensorin$, we have
    \begin{align*}
        \argmax_{\probtensorin} \sbcontraction{\energytensor,\probtensor}+ \sentropyof{\probtensor}
...
\begin{theorem}[Update equations for the naive mean field approximation]
    ...
    \begin{align*}
        \argmax_{\legcoreof{\catenumerator}} \contraction{\{\energytensor\} \cup \{\legcoreofat{\seccatenumerator}{\catvariableof{\seccatenumerator}} \, : \, \seccatenumeratorin\}}
        + \sum_{\catenumeratorin} \sentropyof{\legcoreof{\catenumerator}}
    \end{align*}
    is solved at
...
Let $\graph$ be any hypergraph, we take $\Gamma$ in \probref{prob:meanField} as the exponential family $\mnexpfamily$ of Markov Networks on a hypergraph $\graph$ and define the structured variational approach as
\begin{align}
    \tag{$\mathrm{P}_{\mnexpfamily, \probtensor}$}\label{prob:structuredApproximation}
    \argmax_{\probtensor\in \mnexpfamily} \sbcontraction{\energytensor,\probtensor} + \sentropyof{\probtensor}
\end{align}
By \theref{the:meanFieldIProjection} the structured approximation coincides with the information projection \probref{iProjection} onto the exponential family of Markov Networks given a hypergraph $\graph$.
...
Let us now continue with the discussion of the backward map, which calculates to a maen parameter $\meanparamwith$ a canonical parameter $\canparamwith$, such that the corresponding member of the exponential family reproduced $\meanparamwith$.
...
we now provide a variational characterization of
...
% Backward mapping
Backward mapping coincides with the Maximum Likelihood Estimation Problem \eqref{prob:parameterMaxLikelihood}, when we take the hypothesis to be exponential family $\expfamily$. % for a sufficient statistic $\sstat$.
We show this as a more general statement for M-projections onto exponential families.
...
\begin{theorem}
    ...
    Then $\estcanparam=\backwardmapof{\genmean}$ is the canonical parameter to the solution of the M-projection (Problem~\ref{prob:mProjection}) of any $\gendistribution$ with
    \begin{align*}
...
\begin{proof}
    ...
    The second claim holds, since maximum likelihood estimation is a special instance of M-projection, when projecting $\empdistribution$.
