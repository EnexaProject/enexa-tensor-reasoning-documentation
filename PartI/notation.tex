...
\begin{center}
	\input{PartI/tikz_pics/tensor_networks/tensor_product.tex}
\end{center}
...
\begin{example}{Hadamard Products of Vectors}
	...
	defined by
		\[ \contractionof{\{\vectorofat{\catenumerator}{\catvariable} \, : \, \catenumeratorin\}}{\indexedcatvariable}   = \prod_{\atomenumeratorin} V^\atomenumerator[\indexedcatvariable]\, . \]
...
\begin{definition}\label{def:normation}
	A tensor $\hypercoreat{\catvariablesinset{\nodes}}$ is said to be normable on $\innodes\subset\nodes$, if for any $\atomlegindexof{\innodes}\in\bigtimes_{\node\in\innodes}[\catdimof{\node}]$ we have
		\[ \sbcontraction{\hypercore,\onehotmapof{\atomlegindexof{\innodes}}} > 0 \, . \]
	The normation of a on $\innodes\subset\nodes$ normable tensor is the tensor
	\begin{align*}
		\sbnormationofwrt{\hypercore}{\catvariableof{\outnodes}}{\catvariableof{\innodes}} = 
		\sum_{\atomlegindexof{\innodes}\in\bigtimes_{\node\in\innodes}[\catdimof{\node}]} 
		\onehotmapof{\atomlegindexof{\innodes}} \otimes \frac{
		\sbcontractionof{\hypercore,\onehotmapof{\catindexof{\innodes}}}{\catvariableof{\outnodes}}
		}{
		\sbcontraction{\hypercore,\onehotmapof{\catindexof{\innodes}}}
		} 
	\end{align*}
	where $\outnodes = \nodes / \innodes$.
%	
