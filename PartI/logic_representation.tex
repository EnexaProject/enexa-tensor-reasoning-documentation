...
%In  will define encodings of arbitrary sets based on index interpretation maps.
% Group homomorphism
...
We apply the coordinate and the basis encoding schemes to represent propositional formulas as tensors.
...
%We can then build the basis encoding (\defref{def:functionRepresentation}) of that map to represent the formula (see \figref{fig:formulabencoding}).
...
\begin{proof}
%% Decomposition
    We can decompose basis encodings of formulas into the sum (see \figref{fig:formulabencoding}) % ! Not a tensor network decomposition !
    \begin{align}
        \bencodingofat{\formula}{\formulavar,\shortcatvariables}
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item exactly one node $\secnode$ does not appear as an incoming node of a hyperedge and is called the root node % head node
...
%	The nodes $[\atomorder]$ carry atomic formulas and represent them by identity tensors using the representation scheme of \lemref{lem:atomicFormulaRepresentation}.
    ...
    The claim then follows with \lemref{lem:formulaEncodingDecomposition} applied on $\exformula=\formulaof{\secnode}$.
\end{proof}
