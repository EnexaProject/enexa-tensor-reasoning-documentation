\section{Logical Inference} \label{sec:tensorKB}\label{cha:logicalReasoning}
...
%	\If{$\sbcontractionof{\kb,\exformula}>0$ and $\sbcontraction{\kb,\lnot\exformula}>0$} %Consistency + Redundancy check
	\State $\kb \algdefsymbol \kb\land\exformula$%Add cores of formula tensor $\exformula$ to $\ftensorof{\kb}$ in order to represent $\ftensorof{\kb\cup\{\exformula\}}$
	\EndIf
\end{algorithmic}
...
\begin{center}
	\input{./PartI/tikz_pics/logic_representation/ones_property_ft.tex}
\end{center}
...
\end{align*}
\begin{center}
	\input{./PartI/tikz_pics/logical_reasoning/kb_as_condition.tex}
\end{center}
...
\begin{center}
	\input{./PartI/tikz_pics/logical_reasoning/entailment_check.tex}
\end{center}
