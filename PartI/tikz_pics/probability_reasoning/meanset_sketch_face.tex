...
%    \draw[fill] (P2) circle (0.15cm);
    ...
    % Face normal
    \draw[thick] (Or) -- ($(Or) + -0.6*(-9.5,-4.5)$) node[midway,below] {\small $\canparamwith$};
...
%    \coordinate (int) at ($(Or) + -1.45*(-9.5,-4.5)$);
    ...
    \draw[<->] (M1) --(M2) node [midway, left] {\small $\frac{1}{\|\canparam\|} \cdot \max_{\meanparam\in\genmeanset}\contraction{\meanparam,\canparam}$};
