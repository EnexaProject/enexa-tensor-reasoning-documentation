...
\draw[->] (5.5,-5)--(5.5,-3) node[midway,right] {\tiny $\catvariableof{\neg\exformula}$};

\node[anchor=center] (text) at (5.5,-6) {$\concoreof{\lnot}$};
...
\draw[->] (9.5,-5)--(9.5,-3) node[midway,right] {\tiny $\catvariableof{\exformula\circ\secexformula}$};

\node[anchor=center] (text) at (9.5,-6) {$\concoreof{\circ}$};
...
\begin{scope}[shift={(8,0)}]
	...
	\draw[->] (5.5,-9)--(5.5,-7) node[midway,right] {\tiny $\catvariableof{\secexformula}$};

	\drawatomcore{3.5}{-8}{$\ftensorof{\secexformula}$}
	\drawatomindices{3.5}{-12}	
