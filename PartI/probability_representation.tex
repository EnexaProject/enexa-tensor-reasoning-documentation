...
Exponential families are collections of probability distributions, generalizing the previous discussed graphical models \cite{wainwright_graphical_2008, murphy_probabilistic_2022}.
...
Since we restrict the discussion to finite state spaces, the distribution $\expdist$ is well-defined for any $\canparamwith\in\simpleparspace$.
...
\begin{theorem}[Generic Representation of Exponential Families]
    ...
    \begin{align*}
        \expdistwith =
        \normationof{\{\basemeasurewith\} \cup \{\rencodingofat{\sstatcoordinateof{\selindex}}{\sstatcatof{\selindex},\shortcatvariables} \, : \, \selindexin\}\cup\{\actcoreof{\selindex} \, : \, \selindexin\}}{\shortcatvariables} \, .
    \end{align*}
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        ...
        = \bigotimes_{\selindexin} \actcoreofat{\selindex}{\sstatcatof{\selindex}} \, .
    \end{align*}
...
% Core types
The tensor network representation of an exponential family by \theref{def:expFamilyTensorRep} is a Markov Network consistent of two types of cores.
...
The influence of the contraction is performed by the activation cores $\actcoreofat{\selindex}{\sstatcatof{\selindex}}$, which exploit the computed statistic variable and provide in combination with the relational encoding a factor
\begin{align*}
    \contractionof{\rencodingofat{\sstatcoordinateof{\selindex}}{\sstatcatof{\selindex},\shortcatvariables},\actcoreofat{\selindex}{\sstatcatof{\selindex}}}{\shortcatvariables}
\end{align*}
to the Markov Network reduced to the observed variables $\shortcatvariables$.
When the canonical parameter is vanishing at a coordinate, that is $\canparamat{\indexedselvariable}=0$, then this factor is trivial, since $\actcoreofat{\selindex}{\sstatcatof{\selindex}}=\onesat{\sstatcatof{\selindex}}$ and as a consequence of the directionality of relational encodings we have
\begin{align*}
    \contractionof{\rencodingofat{\sstatcoordinateof{\selindex}}{\sstatcatof{\selindex},\shortcatvariables},\actcoreofat{\selindex}{\sstatcatof{\selindex}}}{\shortcatvariables}
...
\end{center}
Comparing this representation scheme with \theref{def:expFamilyTensorRep}, we conclude as the next corollary, that any member of an exponential family with trivial base measure can be represented by an elementary activation tensors.
...
For elements of the exponential family with general boolean base measure we have with the activation cores constructed in \theref{def:expFamilyTensorRep}
\begin{center}
...
\begin{definition}
    ...
    We call the tensor
    \begin{align*}
        \meanparamwith = \sbcontractionof{\probat{\shortcatvariables},\sencsstatat{\shortcatvariables,\selvariable}}{\selvariable}
    \end{align*}
    the mean parameter tensor to a distribution $\probat{\shortcatvariables}$.
    The set
    \begin{align*}
        \genmeanset = \left\{\contractionof{\probtensor,\sencsstat,\basemeasure}{\selvariable} \, : \,
        0\prec\probtensor , \, \contractionof{\probwith,\basemeasurewith}{\shortcatvariables}
        = \probwith \right\} \, ,
    \end{align*}
    ...
    \end{align*}
    with $\forwardmapof{\canparam} = \sbcontractionof{\expdist,\sencsstat}{\selvariable}$ is called the forward map of the exponential family and inverse, that is a map
    \begin{align*}
...
We now turn our interest towards the characterization of the polytope of realizable mean parameters.
...
Any convex polytope $\genmeanset$ can further be characterized as an intersection of half-spaces, as we state next.

\begin{theorem}
    \label{the:meanPolytopeHalfspaces}
    For any statistic $\sstat$ and base measure $\basemeasure$ there exists a finite collection
    \begin{align*}
        \halfspaceparams
    \end{align*}
...
\end{theorem}
\begin{proof}
    This is a standard result of polyhedral combinatorics, see for a constructive proof \cite{ziegler_lectures_2013}.
\end{proof}
...
Let us now continue with the investigation of the faces of the mean parameter polytope, which we define following Def.~2.1 in \cite{ziegler_lectures_2013}.
...
\begin{definition}
    ...
    \begin{align*}
        \genfacesetof{\facecondset} = \left\{\meanparamwith\in\genmeanset \, : \, \forall_{i\in\mathcal{I}} \, \contraction{\meanparamwith,\normalvecofat{i}{\selvariable}}=\normalboundof{i} \right\}
    \end{align*}
...
\begin{theorem}
    ...
    \[ \genfacesetof{\facecondset} = \argmax_{\meanparam\in\genmeanset} \contraction{\canparamwith,\meanparamwith}  \, . \]
...
\begin{proof}
    ...
    We use that the face is non-empty, and thus there is a $\meanparamwith$ with
    ...
    This is equal to $\meanparam\in\genfacesetof{\facecondset}$.
\end{proof}
...
\begin{theorem}
    ...
    \begin{align*}
        \argmax_{\meanparam\in\genmeanset} \contraction{\canparamwith,\meanparamwith}
...
\begin{proof} % Not a smooth proof, improve!
    ...
    \end{align*}
    Further, since the contraction with $\canparamwith$ is linear, the set $\argmax_{\meanparam\in\genmeanset} \contraction{\canparamwith,\meanparamwith}$ is contained in the boundary of the polytope $\genmeanset$.
...
\begin{theorem}[Exponential Representation of Markov Networks]\label{the:markovNetworkExponentialFamilies}
    ...
    Then, the Markov Network is in the member of the exponential family with trivial base measure, sufficient statistic $\sstatof{\graph}$ and parameters $\canparam$.
\end{theorem}
\begin{proof}
...
To prepare for reasoning on data in the next section, we now derive tensor network representation for empirical distributions, which are defined based on observed states $\dataset$ of a factored system.
