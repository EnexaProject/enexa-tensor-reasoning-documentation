...
%\subsect{Mean Parameters of exponential families}

%Mean parameters are an alternative way to represent members of exponential families.
\red{Given a statistic $\sstat$, we define a mean parameter to any distribution by the expectation of the statistic.}

\begin{definition}
    \label{def:meanPolytope}
    Let there be a statistic $\sencsstatwith$ and a boolean base measure $\basemeasurewith$.
    We call the tensor
    \begin{align*}
        \meanparamwith
...
\red{We in the reminder of this section prepare for this discussion and investigate and derive tensor network representations for distributions having sufficient statistics $\sstat$, depending on their corresponding mean parameter in the polytope.
}
...
\subsect{Representation by convex hulls and intersecting half-spaces}
...
\begin{theorem}
    ...
    For any minimal statistics $\sstat$ with respect to a boolean base measure $\basemeasure$ (see \defref{def:minimalStatistic}) we have for some $\meanparamwith$ that $\meanparamwith\in\sbinteriorof{\genmeanset}$ if and only if there is a positive distribution with respect to $\basemeasure$ such that
    \begin{align*}
         \meanparamwith = \contractionof{\probtensor,\sencsstat}{\selvariable} \, .
    \end{align*}
\end{theorem}
\begin{proof}
    \proofrightsymbol:
...
We can define face measures as follows.

\begin{definition}
    \label{def:faceMeasure}
    The base measure to the face $\genfacesetof{\facecondset}$ of $\meanset$ is the boolean tensor
    \begin{align*}
...
This shows that faces are mean parameter polytopes with respect to refined base measures.
...
\begin{theorem}[Face measure characterization]\label{the:faceMeasureCharacterization}
...
To prepare for reasoning on data in the next chapter, we now derive tensor network representation for empirical distributions, which are defined based on observed states $\dataset$ of a factored system.
