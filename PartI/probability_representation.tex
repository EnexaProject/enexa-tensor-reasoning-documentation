...
In combination with \theref{the:rencodingDirected}, which states that basis encodings are directed, we get that any basis encoding of a function is a conditional probability tensor.
...
In the proof of \theref{the:expFamilyTensorRep} we have observed, that the basis encoding $\rencodingofat{\sstat}{\sstatcatof{[\seldim]},\shortcatvariables}$ of the statistics decomposed into a tensor network of basis encodings $\rencodingofat{\sstatcoordinateof{\selindex}}{\sstatcatof{\selindex},\shortcatvariables}$ to the coordinate of the statistic.
...
First, we refer to the basis encodings $\rencodingof{\sstatcoordinateof{\selindex}}$ of the coordinates of a statistic as computation cores.
...
Let us now investigate tensor network representations of face measures, based on the basis encoding $\rencodingof{\sstat}$ of a statistic.
% Vertices
