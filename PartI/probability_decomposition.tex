...
We further restrict to boolean base measures, which satisfy $\contraction{\basemeasure}\neq0$
...
\begin{definition}[Minimal]\label{def:minimalStatistics}
	We say that a statistic $\sstat$ is minimal with respect to a boolean base measure $\basemeasure$, if there is no pair of a nonvanishing vector $\canparamat{\selvariable}$ and a scalar $\lambda\in\rr$ with
		\[ \contractionof{\sencsstat,\canparam,\basemeasure}{\shortcatvariables} = \lambda\cdot\basemeasureat{\shortcatvariables} \, . \]
\end{definition}
...
\begin{definition}\label{def:meanForwardBackward}
	...
	The set 
		\[ \meansetof{\sstat,\basemeasure} = \left\{\contractionof{\probtensor,\sencsstat,\basemeasure}{\selvariable} \, : \, 0\prec\probtensor , \, \contractionof{\probtensor,\basemeasure}{\shortcatvariables} 
...
\begin{theorem}\label{the:meanPolytopeHalfspaces}
	For any statistic $\sstat$ and base measure $\basemeasure$ there exists a collection $\left( (a_i[\selvariable],b_i) \, : \, i \in[n]\right)$ where $a_i[\selvariable]$ a vector and $b_i\in\rr$ for all $i\in[n]$ such that
	\begin{align*}
		\meansetof{\sstat,\basemeasure}
		= \left\{\meanparamat{\selvariable} \, : \, \forall_{i\in[n]} \, \contraction{\meanparamat{\selvariable},a_i[\selvariable]}\leq b_i \right\} \, . 
	\end{align*}
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:meanPolytopeFaces}
	Given a mean parameter polytope $\meansetof{\sstat,\basemeasure}$ in the half space representation of Theorem~\ref{the:meanPolytopeHalfspaces}, and any subset $\mathcal{I}\subset[n]$ we say that the set
	\begin{align*}
		 \left\{\meanparamat{\selvariable} \, : \, \forall_{i\in\mathcal{I}} \, \contraction{\meanparamat{\selvariable},a_i[\selvariable]} = b_i \right\} 
	\end{align*}
...
\begin{example}[The minterm exponential family]\label{exa:mintermExpFamily}
	...
	and $\meansetof{\sstat}$ coincides with the set of probability tensors.
...
\begin{definition}\label{def:dataMap}
	...
	\end{align*}
	where by $\datacore$ we denote the relational encoding (see Definition~\ref{def:functionRepresentation}) of the sample selector map, and the distributed variables $\shortcatvariables$ are the head variables of the relational encoding.
\end{definition}
