...
%	\[ \probtensor_{\catindices} = \probof{\randomxof{\atomenumerator} = \catindexof{\atomenumerator} \, : \, \atomenumeratorin}  \, . \]
...
%	\begin{align*}
%	\condprobof{
%	\randomxof{\atomenumerator},\ldots,\randomxof{\atomorder}
%	}{
...
%	\begin{align*}
%		%\contractionof{\{
%			\condprobof{\randomxof{\atomenumerator},\ldots,\randomxof{\atomorder-1}}{\randomxof{1},\ldots,\randomxof{\atomenumerator-1}}
%		%\}}{} 
%		= \contractionof{\{
%			\condprobof{\randomxof{\atomenumerator+1},\ldots,\randomxof{\atomorder-1}}{\randomxof{1},\ldots,\randomxof{\atomenumerator-1}},
%			\condprobof{\randomxof{\atomenumerator}}{\randomxof{1},\ldots,\randomxof{\atomenumerator-1}}	
%		\}}{
...
%		\condprobof{\randomxof{\atomenumerator},\ldots,\randomxof{\atomorder-1}}{\randomxof{1},\ldots,\randomxof{\atomenumerator-1}}
%		= \contractionof{\{
%			\condprobof{\randomxof{\secatomenumerator}}{\randomxof{1},\ldots,\randomxof{\atomenumerator-1}} \, : \, \secatomenumerator = \atomenumerator, \atomenumerator +1 , \ldots \atomorder-1
%		\}}{
%			\catvariableof{[\atomorder]} 
%		} \, .
%	\end{align*}
...
\begin{theorem}[Markov Chain]\label{the:MarkovChain}
	...
	\end{align*}	
%		\[ \probof{\randomxof{0},\ldots,\randomxof{\tdim-1}} = %\probof{\randomxof{1}} 
%		\prod_{\tenumeratorin} \condprobof{\randomxof{\tenumerator}}{\randomxof{\tenumerator-1}} \, . \] 
...
\begin{proof}
	...
	\begin{align*}
		...
		%= \contractionof{\{\probof{\randomxof{0}} \prod_{\tenumeratorin, \tenumerator>1} \condprobof{\randomxof{\tenumerator}}{\randomxof{0:\tenumerator}}\}{\catvariableof{[\tdim]}} \, . 
	\end{align*}
...
\begin{definition}
	...
	Let $\graph=(\nodes,\edges)$ be a directed acyclic hypergraph with edges of the form 
		\[ \edges = \{(\parentsof{\node},\{\node\}) \, : \, \nodein\} \, . \]
%	and for each node $\node\in\nodes$ a random variable $\randomxof{\node}$.
...
%\begin{align*}
%	\probtensorof{\graph} = \sbcontractionof{\{ \condprobof{\randomxof{\node}}{\randomxof{\parentsof{\node}}} \, : \, \node\in\nodes\}}{\nodes} \, . 
%\end{align*}
...
\red{Overwork: Further integration of selection encodings!}

%We here treat linear exponential Families (log-linear models) defined by sufficient statistics.
...
\begin{definition}
	...
	We call the tensor
		\[ \meanparam = \sbcontractionof{\expdist,\sencodingof{\sstat}}{\selvariableof{\sstat}} \]
	...
	with $\forwardmap(\canparam) = \sbcontractionof{\expdist,\sencodingof{\sstat}}{\selvariableof{\sstat}}$ is called the forward map of the exponential family and any map
		\[ \backwardmap : \imageof{\forwardmap} \rightarrow \simpleparspace\]
	with $\expdistof{(\sstat,\backwardmap(\forwardmap(\canparam)),\basemeasure)} = \expdist$ for any $\canparam\in\rr^{\statorder}$ a backward map.
\end{definition}

% Polytope
The image $\meanset$ of the forward map $\imageof{\forwardmap}$ is the interior of the convex polytope $\meanset$.
It contains any mean parameters $\sbcontractionof{\probtensor,\sstat}{\selvariableof{\sstat}}$ realizable by any probability distribution $\probtensor$ \cite{wainwright_graphical_2008}.
