...
%
We note, that when choosing for a formula $\kb$ the uniform distribution $\probat{\shortcatvariables}=\sbnormationof{\shortcatvariables}{\kbvar=1}$ among its models, then probabilistic entailment $\probtensor\models\exformula$ of a propositional formula $\exformula$ is by \theref{the:probEntailment} equivalent to $\kb\models\exformula$.
...
Exponential families have a tensor network representation by a Markov Network (see \theref{def:expFamilyTensorRep}).
