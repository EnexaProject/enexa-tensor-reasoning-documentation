...
\begin{definition}[Categorical Constraint and Atomization Variables]\label{def:catConAtomVar}
    Given a list $\catvariableof{0},\ldots,\catvariableof{\catdim-1}$ of boolean variables and a categorical variable $\catvariable$ with dimension $\catdim$ a categorical constraint is a tensor $\categoricalmap[\catvariableof{[\catdim]},\catvariable]$ defined as
    \begin{align*}
        \categoricalmapat{\indexedcatvariableof{[\catdim]},\indexedcatvariable}
%		 \categoricalmap(\catindex,\catindexof{\variableset})
        = \begin{cases}
              1 & \ifspace \shortcatvariables = \onehotmapof{\catindex} \quad \Big(\text{i.e. }\forall \catenumerator\in[\catdim]: \big(\catindex=\catenumerator\big) \Leftrightarrow \big(\catindexof{\catenumerator}=1\big) \Big)\\
              0 & \text{else} \, .
        \end{cases}
    \end{align*}
...
\begin{align*}
    ...
    = \begin{cases}
          1 & \text{if} \quad \catindex=\catenumerator \quad \text{and} \quad \catindexof{\catenumerator} = 1 \\
          1 & \text{if} \quad \catindex\neq\catenumerator \quad \text{and} \quad \catindexof{\catenumerator} = 0 \\
          0 & \text{else} \, .
    \end{cases}
\end{align*}
...
\begin{proof}
    ...
    \]
    If $\catindexof{\variableset} = \onehotmapof{\catindex}$ for some $\catindex\in[\cardof{\variableset}]$ we have $\categoricalmap[\indexedcatvariableof{\variableset},\indexedcatvariable] = 1$ and thus
    \[ \contractionof{\hypercoreat{\shortcatvariables},\categoricalmap}{\indexedcatvariables}  =  \hypercoreat{\shortcatindices}  \, . \]
    If there is no such $\catindex$ then $\categoricalmap[\indexedcatvariableof{\variableset},\indexedcatvariable] = 0$ and
    \begin{align*}
        \contractionof{\hypercoreat{\shortcatvariables},\categoricalmap}{\indexedcatvariables}  = 0 \, . & \qedhere
    \end{align*}
\end{proof}
