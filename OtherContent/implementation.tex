...
These are implementations of the matrix representation of \remref{rem:matSotrageBasPlus}.
...
Relational encodings (see \defref{def:functionRelationEncoding}) of functions $\exfunction:\facstates \rightarrow \secfacstates$ are created by
\begin{lstlisting}
...
\figref{tab:connectives} lists the supported logical connectives (see \defref{def:connectives}), which are implemented in \inlinecode{representation.basisplus_calculus}.
...
\begin{example}[Representation of a member of an exponential family]
...
More formally, the production rules are formalized in \figref{tab:bnNeurons} by the extension of the Backus-Naur form in \figref{tab:bnFunction}.
