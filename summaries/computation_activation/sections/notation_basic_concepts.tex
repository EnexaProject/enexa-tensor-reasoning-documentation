...
%For a tensor $\hypercoreat{X_{[2]}} \in \mathbb{R}^{m_0}\otimes\mathbb{R}^{m_1}$ and any $\secexrandindin$ we depict the slice $\hypercoreat{X_0|X_1=x_1}\in\mathbb{R}^{m_0}$ and $\hypercoreat{X_0|X_1}\in\mathbb{R}^{m_0}\otimes\mathbb{R}^{m_1}$ defined by a normalization operation as
%\begin{center}
...
\begin{definition}[Basis encoding of maps between state sets]
    ...
    \end{align*}
\end{definition}
Basis encodings are directed tensors (see~\cite[Theorem 14.10]{goessmann2025}) and are thus depicted as decorations of directed edges in hypergraphs: \janina{Explain the arrows. Haven't introduced directed graphs yet.}
\begin{center}
...
\begin{align*}
    \bencodingofat{\statesetfunction}{\headvariables=y_{[p]},\shortcatvariables=x_{[d]}}
    = \begin{cases}
          1 & \ifspace q(x_{[d]})=y_{[p]}\\
          0 & \text{else}
    \end{cases}
\end{align*}
%\janina{Put basis encoding of propositional formula as in logic section as an example here? Hmm did not define them yet}



%\janina{The definition includes conditional tensors. not introduced yet. Also no word about probability distributions yet.}
