...
Representing booleans by elements in $\{0,1\}$ leads to the problem, that negation is an affine transformation and can not be represented by multilinear tensors~\cite[Section 4.1.1]{goessmann2025}.
...
\begin{align}
    ...
    = \formulaat{\shortcatvariables} \otimes \tbasisat{\headvariableof{\formula}}
    + \lnot\formulaat{\shortcatvariables} \otimes \fbasisat{\headvariableof{\formula}} \, .
\end{align}
...
Classically (see e.g. \cite{russell_artificial_2021}), one defined entailment in propositional logics, based on the unsatisfiability of $\kb\land\lnot\exformula$.
...
\begin{theorem}[Monotonocity of Propositional Logics]\label{the:monotonicityPL}
