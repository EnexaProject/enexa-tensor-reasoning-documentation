...
\red{Thus the polytope of mean parameters depends on the base measure only through its support.}
...
\red{More general, we define face representability with respect to any hypergraph}
...
\end{lemma}
\begin{proof}
    Generalization of the above for CP.
\end{proof}
...
We notice that $\kcoreofat{\facecondset}{\headvariables}$ in \theref{the:faceMeasureCharacterization} is a sparse tensor with basis $\cpformat$ rank $\cardof{\mathcal{N}^{\facecondset}_{\sstat,\basemeasure}}$ (see \charef{cha:sparseRepresentation}).
\begin{align*}
