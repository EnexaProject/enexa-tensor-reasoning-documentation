...
We restate \theref{the:functionImageDecompositionContraction} as a basis CP decomposition bound.
...
\begin{theorem}\label{the:functionDecompositionBasisCP}
	...
	Then $\rencodingofat{\exfunction}{\shortcatvariables,\catvariable}$ has a basis CP decomposition with decomposition index $\catvariable$, trivial slices $\onesat{\catvariable}$ leg vectors $\rencodingofat{\exfunctionof{\atomenumerator}}{\catvariableof{\atomenumerator},\catvariable}$.
\end{theorem}
\begin{proof}
...
\begin{example}{Empirical distributions, see \theref{the:empCPRep}}\label{exa:empDistCP}
...
\begin{example}{Exponential families}\label{exa:expFamCP}
	The statistic has a CP decomposition with rank by the cardinality of states
\end{example}
