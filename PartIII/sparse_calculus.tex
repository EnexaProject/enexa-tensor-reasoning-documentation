...
\begin{proof}[Proof of \theref{the:CPrankContractionBound}]
    ...
    We then have
    \begin{align}\label{eq:cprankContractionPartition}
...
While \theref{the:rencodingBasCP} provides a $\cpformat$ rank bound based on the domain factored system, we can also show in the next theorem a bound using the structure of the image.
...
%\subsubsect{Construction by averaging the incoming legs}
...
\end{theorem}
\begin{proof}
    Reinterpretation of the contractions in the decomposition claimed in \theref{the:functionImageDecompositionContraction}.
\end{proof}
...
%\sect{Manipulations of Binary $\cpformat$ decomposition}\label{sec:BinaryCPManipulation}
%
%Since the coordinates on the legs are binary, operations like contractions, slicing and marginalization are especially efficient given binary $\cpformat$ decompositions.
...
% $\cpformat$ decompositions
%Analogously to monomial decompositions, quadratic decompositions have an equivalence in a $\cpformat$ decomposition of $\hypercore$.
