...
\begin{definition}[Partial ordering of tensors]\label{def:partialOrder}
...
In \charef{cha:sparseTC} we will apply \theref{the:functionImageDecompositionContraction} in \theref{the:functionDecompositionBasisCP} to show sparse basis $\cpformat$ decompositions to $\rencodingof{\exfunction}$.
...
\begin{example}[Relational encodings as basis encodings]
...
\end{definition}
We always have $\onesat{\shortcatvariables}\in\subspaceof{\exfunction}$, when choosing $\actcoreat{\indvariableof{\outsymbol}}=\onesat{\indvariableof{\outsymbol}}$.
...
\subsect{Cartesian products of functions}

Since always $\onesat{\shortcatvariables}\in\subspaceof{\exfunction}$, we have
\begin{align*}
...
\begin{example}[Subspace of statistics]
    The realizable distributions defined in \defref{def:realizableStatDistributions} given a statistic function are then
    \begin{align*}
...
\subsect{Composition of functions}
