...
We have motivated such constraints by conditional distributions, see \defref{def:condIndependence}, and refered to Markov Networks (see \defref{def:markovNetwork}) satisfying these by Bayesian Networks (see \defref{def:bayesianNetwork}).
...
We first show that any normable tensor is the contraction of its normalization and an accompanying contraction, which generalizes the Bayes \theref{the:bayes} towards more generic normable tensors.
